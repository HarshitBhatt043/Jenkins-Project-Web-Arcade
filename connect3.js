...
class Tile {
        ...
        if (selectedTile === "DONTSELECT");
          ...
          if (
            (selectedTile.x === this.x &&
              Math.abs(selectedTile.y - this.y) === 1) ||
            (selectedTile.y === this.y &&
              Math.abs(selectedTile.x - this.x) === 1)
          ) {
            let lastPositions = {
              me: [this.x, this.y],
            ...
            if (!startMurderSpree(200)) {
              ...
              setTimeout(() => {
                lastPositions.selectedTile[2].moveTo(
                  lastPositions.selectedTile[0],
                  lastPositions.selectedTile[1]
                );
  ...
  remove(animate = true, addNewItem = true) {
    ...
    for (let tempY = this.y; tempY--; )
      tileElems[tempY][this.x].moveTo(this.x, tempY + 1);
    if (addNewItem) {
  ...
  moveTo(x, y) {
    if (tileElems[this.y][this.x] === this)
      (tileElems[this.y][this.x] = null), (tiles[this.y][this.x] = null);
...
function startMurderSpree(wasRightAfterTransition = false) {
  ...
  while ((foundMatches = checkForMatches(tiles)).length) {
    for (let i = foundMatches.length; i--; )
      for (let j = foundMatches[i].length; j--; ) {
...
function checkForMatches(tiles) {
  function getCoords(x, y, ywise = true, frontOffset = 0, sideOffset = 0) {
    return [
      x + (ywise ? sideOffset : frontOffset),
      y + (ywise ? frontOffset : sideOffset),
    ];
  }
  ...
  function areSameColour(tile1, tile2) {
    // -1 is wildcard; NaN is a unique tile
      ...
      for (let i = matches.length; i--; ) {
        ...
        for (let j = 1; j < 6; j++) {
          ...
          if (
            !leftIsDone &&
            sideCoord - j >= 0 &&
            areSameColour(colour, getTile(leftCoords))
          )
            sideMatches.push(leftCoords);
          else leftIsDone = true;
          if (
            !rightIsDone &&
            sideCoord + j < 8 &&
            areSameColour(colour, getTile(rightCoords))
          )
            sideMatches.push(rightCoords);
...
while ((foundMatches = checkForMatches(tiles)).length) {
  for (let i = 3; i--; )
    tiles[foundMatches[0][i][1]][foundMatches[0][i][0]] = Math.floor(
