function CompatibilityCheck() {
  ...
  // Show warnings if needed.
    ...
    if (
      !confirm(
        "Please note that Unity WebGL is not currently supported on mobiles. Press Ok if you wish to continue anyways."
      )
    )
      window.history.back();
  } else if (
...
if (typeof window.onerror != "function") {
  window.onerror = function UnityErrorHandler(err, url, line) {
    ...
    if (err.indexOf("UnknownError") != -1) return;
    ...
    if (
      err.indexOf("Invalid array buffer length") != -1 ||
      err.indexOf("out of memory") != -1
    ) {
      alert(
