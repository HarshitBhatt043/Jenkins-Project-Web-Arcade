...
function dg_initialize() {
  ...
  if (!show_loading_screen) dg_hide_loading();
  else resizeLoadingImg();
  ...
  if (show_loading_screen)
    if (imgLoad && imgBar)
      if (game_width > game_height) {
        ...
        imgLoad.style.left =
          (wbr - (hbr / game_height) * game_width) / 2 + "px";
        imgLoad.style.top = 0 + "px";

        imgBar.style.height = (hbr / game_height) * preloader_bar_height + "px";
        imgBar.style.width =
          (loadingProgress * preloader_bar_width * hbr) / game_height + "px";
        imgBar.style.left =
          (wbr - (hbr / game_height) * preloader_bar_width) / 2 + "px";
...
function dg_hide_loading() {
  ...
  if (canvas_parent_div) canvas_parent_div.style.display = "block";
}
...
function dg_set_button_params(buttonID, _x, _y, _width, _height) {
  ...
  if (b) {
    ...
    b.style.left =
      (wbr - scale_scr * game_width) / 2 + _x - _width * 0.5 + "px";
    b.style.top =
      (hbr - scale_scr * game_height) / 2 + _y - _height * 0.5 + "px";
