...
function initGDApi() {
  ...
  if (!initialized) {
    // Api will be initialized once, so preroll is shown once either
...
function gdApi_showBanner() {
  if (initialized) gdApi.showBanner();
}
...
function gdApi_customLog(_Key) {
  if (initialized) gdApi.customLog(_Key);
}
