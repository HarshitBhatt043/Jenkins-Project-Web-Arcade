...
InputController.getInstance = function () {
  if (!InputController._instance)
    console.error("InputController is not initialized yet");
...
InputController.prototype.update = function (dt) {
  if (this._keyboardInputActive) {
    this.app.fire(
      EventTypes.GameInput.UPDATED,
      Math.PI / 2 - this._getKeyboardInputAzimuth()
    );
    this.app.fire(
      EventTypes.GameInput.KEYBOARD_UPDATED,
      Math.PI / 2 - this._getKeyboardInputAzimuth()
    );
  }

  if (this.activePointer.active) {
    const velocityThreshold = this.activePointer.touch
      ? GameConfig.getAttribute("input", "frameVelocityThreshold")
      : GameConfig.getAttribute("input", "mouseFrameVelocityThreshold");
    const velocityDecay = this.activePointer.touch
      ? GameConfig.getAttribute("input", "frameVelocityDecay")
      : GameConfig.getAttribute("input", "mouseFrameVelocityDecay");
...
InputController.prototype._updatePointerPosition = function (x, y) {
  ...
  const _distanceThreshold = this.activePointer.touch
    ? GameConfig.getAttribute("input", "touchDistanceThreshold")
    : GameConfig.getAttribute("input", "mouseDistanceThreshold");
  const _frameVelocity = new pc.Vec2(
  ...
  this.activePointer.velocity = this.activePointer.velocity.add(
    _frameVelocity.scale(
      this.activePointer.touch
        ? GameConfig.getAttribute("input", "frameDistanceTolerance")
        : GameConfig.getAttribute("input", "mouseFrameDistanceTolerance")
    )
  );
  ...
  this.activePointer.azimuth = Math.atan2(
    this.activePointer.velocity.y,
    this.activePointer.velocity.x
  );
};
...
InputController.prototype._isKeyboardInputActive = function () {
  return (
    this.app.keyboard &&
    [pc.KEY_LEFT, pc.KEY_RIGHT, pc.KEY_DOWN, pc.KEY_UP].some((key) =>
      this.keyStates.get(key)
    )
  );
};
...
InputController.prototype.onTouchEnd = function (event) {
  if (this.activePointer.active) {
    const foundTouch = event.changedTouches.find(
      (touch) => touch.id === this.activePointer.id
    );
    if (foundTouch) {
...
InputController.prototype.onTouchMove = function (event) {
  if (this.activePointer.active) {
    ...
    if (foundTouch) {
      ...
      this.app.fire(
        EventTypes.GameInput.TOUCH_UPDATED,
        Math.PI / 2 - this.activePointer.azimuth
      );
      this.app.fire(
        EventTypes.GameInput.TOUCH_MOVE,
        foundTouch.x,
        foundTouch.y
      );
    }
...
EventTypes.AWARD_PLAYER_STICKMAN_WITH_BRICKS =
  "gameplay:awardPlayerStickmanWithBricks";
...
EventTypes.Platform = {
  ...
  GENERATE_BRICKS_WHEN_STICKMAN_ENTERED:
    "platform:generateBricksWhenStickmanEntered",
  DESTROY_BRICKS_WHEN_STICKMAN_LEAVED:
    "platform:destroyBricksWhenStckmanLeaved",
};
...
GameConfig.attributes.add("gameplay", {
  ...
  schema: [
    ...
    },
    {
      ...
      max: 1,
      description:
        "Amount of bricks of some color that may be generated once stickman enters a platform. 0.4 means 40% of max platfomr capacity",
    },
    {
...
GameConfig.attributes.add("AI", {
  ...
  schema: [
    ...
    },
    {
      ...
      type: "number",
      description:
        "Max horizontal (X/Z) distance to target to consider target as reached",
      default: 0.5,
    },
    {
      ...
      type: "number",
      description:
        "Max vertical (Y) distance to target to consider target as reached. Recommended not less than 0.8",
      default: 0.85,
    },
    {
    ...
    },
    {
      ...
      description:
        "How ofter can AI bot search for dropped bricks, in milliseconds",
      type: "number",
      default: 500,
    },
    {
  ...
  if (GameConfig.instance[category] === undefined) {
    ...
    if (key !== undefined) {
      if (GameConfig.instance[category][key] === undefined) {
        console.warn(
          "GameConfig doesn't contain key  " + category + " > " + key
        );
        return null;
      } else {
...
GameConfig.setAttribute = function (category, key, value) {
  if (GameConfig.instance[category] === undefined) {
    ...
    console.warn(
      "GameConfig.setAttribute: invalid key  " + category + " > " + key
    );
    return;
  } else {
...
Follow.prototype.followTarget = function (inertia) {
  ...
  if (this.target.tags.has("stickman")) {
    ...
    if (numBricks > this.minBricksToIncreaseDistance) {
      deltaPos.scale(
        1 +
          (numBricks - this.minBricksToIncreaseDistance) *
            this.extraDistancePerBrick
      );
    }
    const maxDistance = pc.platform.mobile
      ? this.maxMobileDistance
      : this.maxDistance;
  ...
  this.entity.cameraDisplacement = this.vec
    .clone()
    .sub(targetPos)
    .sub(deltaPos);
};
...
Utils.tweenText = function (
  ...
  textElement.element.text =
    "" + (prefix || "") + Math.round(initialValue) + (suffix || "");
  textElement
    ...
    .on("update", function () {
      textElement.element.text =
        "" +
        (prefix || "") +
        Math.round(textElement.element.textValue) +
        (suffix || "");
    })
    .start();
...
Utils.rotateTo = function (currentAngle, targetAngle, lerp) {
  ...
  if (
    ...
    Math.abs(targetAngle - currentAngle) >= 1.5707963267948966 - lerp
  ) {
...
BricksSpawner.prototype.initialize = function () {
  ...
  if (!this.entity.brickSpawner)
    return console.warn(
      "Platform " + this.entity.path + " has no brick spawn zones defined"
    );
...
BricksSpawner.prototype._addEventListeners = function () {
  ...
  this.app.on(
    EventTypes.GameSession.SESSION_STARTED,
    this._onSessionStarted,
    this
  );
  this.entity.on(
    EventTypes.Platform.GENERATE_BRICKS_WHEN_STICKMAN_ENTERED,
    this.generateBricksWhenStickmanEntered,
    this
  );
  this.entity.on(
    EventTypes.Platform.DESTROY_BRICKS_WHEN_STICKMAN_LEAVED,
    this.destroyBricksWhenStickmanLeaved,
    this
  );
  this.entity.on(
    EventTypes.Platform.GENERATE_BRICK_AT_EMPTY_SPOT,
    this.generateBrickAtEmptySpot,
    this
  );
};
...
BricksSpawner.prototype.calculateSpawnBounds = function () {
  ...
  const aabbs = this.entity.brickSpawner.children
    .filter((entity) => entity.tags.has("spawn-zone"))
    .map((zone) => zone.model.model.meshInstances[0].aabb);
  if (aabbs.length > 0) {
...
BricksSpawner.prototype.spawnBrickSpots = function () {
  this.entity.spawnZonesAABBs = this.entity.brickSpawner.children
    .filter((entity) => entity.tags.has("spawn-zone"))
    .map((zone) => new pc.OrientedBox(zone.getWorldTransform()));
  this.entity.skipZonesAABBs = this.entity.brickSpawner.children
    .filter((entity) => entity.tags.has("skip-zone"))
    .map((zone) => new pc.OrientedBox(zone.getWorldTransform()));
...
BricksSpawner.prototype.createBrickSpots = function () {
  ...
  for (let i = -rows; i <= rows; i++) {
    for (let j = -cols; j <= cols; j++) {
      testPoint.set(
        containerPosition.x + this.displacement.x + i * this.step.x,
        containerPosition.y + this.displacement.y,
        containerPosition.z + this.displacement.z + j * this.step.y
      );
...
BricksSpawner.prototype.placeBrickSpot = function (point) {
  ...
  this.testCorners[0].set(
    point.x - brickHalfWidth,
    point.y,
    point.z - brickHalfHeight
  );
  this.testCorners[1].set(
    point.x + brickHalfWidth,
    point.y,
    point.z - brickHalfHeight
  );
  this.testCorners[2].set(
    point.x - brickHalfWidth,
    point.y,
    point.z + brickHalfHeight
  );
  this.testCorners[3].set(
    point.x + brickHalfWidth,
    point.y,
    point.z + brickHalfHeight
  );
  ...
  for (let skipZone of this.entity.skipZonesAABBs) {
    if (
      skipZone.containsPoint(point) ||
      this.testCorners.some((testPos) => skipZone.containsPoint(testPos))
    ) {
      return;
    }
  }
  ...
  for (let spawnZone of this.entity.spawnZonesAABBs) {
    if (spawnZone.containsPoint(point)) {
      ...
      if (
        this.testCorners.every((testPos) => spawnZone.containsPoint(testPos))
      ) {
        // precise check
        this.entity.createBrickSpotAt(
          point.x,
          point.y + this.brickSize.y / 2,
          point.z
        );
        return;
      }
  ...
  if (this.entity.level.firstPlatform === this.entity) {
    if (!GameConfig.getAttribute("debug", "demoMode")) {
      ...
      this.entity.brickSpots.forEach((bs, index) =>
        bs.createBrick(
          this.app.colorsStorage.getRandomColor(),
          Math.random() *
            GameConfig.getAttribute("gameplay", "initialBricksSpawningDuration")
        )
      );
    }
  }
...
BricksSpawner.prototype.generateBrickAtEmptySpot = function (color) {
  ...
  const currentBricksAmount =
    this.entity.getBrickSpotsWithColoredBricks(color).length;
...
BricksSpawner.prototype.generateBricksWhenStickmanEntered = function (
  stickman
) {
  if (this.entity.level.firstPlatform === this.entity) return; //do not generate bricks on first platform

  const numAlreadyPresentedBricks = this.entity.getBrickSpotsWithColoredBricks(
    stickman.color
  ).length;
  const totalBricksToCreate = this.entity.getMaxPossibleBricksOfColor(
    stickman.color
  );
  for (let i = numAlreadyPresentedBricks; i < totalBricksToCreate; i++) {
    this.generateRandomBrick(
      stickman.color,
      ((i - numAlreadyPresentedBricks) /
        (totalBricksToCreate - numAlreadyPresentedBricks)) *
        GameConfig.getAttribute("gameplay", "initialBricksSpawningDuration")
    );
  }
};
...
BricksSpawner.prototype.generateRandomBrick = function (color, delay) {
  ...
  const otherColorsBrickSpots =
    this.entity.getColoredBrickSpotsSkipingColor(color);
...
Backpack.prototype._assignEntityMethods = function () {
  ...
  this.entity.dropBricks = function () {
    ...
    if (droppedBricks > 0) {
      this.delayedCall(
        GameConfig.getAttribute("brick", "maxActivationDelay") + 100,
        () => {
          this._app.fire(
            EventTypes.STICKMAN_DROPPED_BRICKS,
            droppedBricks,
            dropPosition
          );
        }
...
Backpack.prototype.onBrickPickedUp = function (brick, instantly) {
  ...
  const pendingDestroy =
    ...
    GameConfig.getAttribute("gameplay", "maxBackpackCapacity");
  ...
  if (instantly) {
    ...
    brick
      .tween(brick.getLocalPosition())
      .to(targetPosition, 0.2 + this.entity.bricks.length * 0.0075, pc.BackOut)
      .start();

    brick
      .tween(brick.getLocalRotation())
      .rotate(
        new pc.Vec3(180, 0, 180),
        0.2 + this.entity.bricks.length * 0.0075,
        pc.SineInOut
      )
      .on("complete", () => {
...
Brick.prototype._removeEventListeners = function () {
  if (
    this.entity &&
    this.entity.triggerVolume &&
    this.entity.triggerVolume.collision
  )
    this.entity.triggerVolume.collision.off(
      "triggerenter",
      this.onTriggerEnter,
      this
    );
  ...
  if (this.entity)
    this.entity.off(EventTypes.ENTITY_CACHED, this.onCached, this);
  if (this.entity)
    this.entity.off(
      EventTypes.ENTITY_REUSED_FROM_CACHE,
      this.onReusedFromCache,
  ...
  Object.defineProperty(this.entity, "color", {
    ...
    set(value) {
      if (this._color !== value) {
        this._color = value;
        this.brickModel.model.model.meshInstances[0].material =
          this._app.colorsStorage.getMaterial(this._color);
        if (this.rigidbody) {
          this.rigidbody.group = this._app.colorsStorage.getBrickCollisionGroup(
            this._color
          );
          this.rigidbody.mask = this._app.colorsStorage.getBrickCollisionMask(
            this._color
          );
        }
      }
  ...
  Object.defineProperty(this.entity, "active", {
    ...
    set(value) {
      if (value !== this._active) {
        this._active = value;
        this._app.fire(
          value ? EventTypes.BRICK_ACTIVATED : EventTypes.BRICK_DEACTIVATED,
          this
        );
      }
    },
...
Brick.prototype._assignEntityMethods = function () {
  ...
  this.entity.drop = function () {
    ...
    this._activationDelayedCall = this.delayedCall(
      GameConfig.getAttribute("brick", "maxActivationDelay"),
      () => {
  ...
  this.entity.tweenAppearing = function (delay) {
    ...
    this.tween(this.getLocalScale())
      .to(
        this._initialScale,
        GameConfig.getAttribute("brick", "appearingDuration"),
        pc[GameConfig.getAttribute("brick", "appearingEasing")]
      )
      .delay(delay || 0)
      .start();
  }.bind(this.entity);
...
Brick.prototype._initializeColliders = function () {
  this.entity.triggerVolume.collision.on(
    "triggerenter",
    this.onTriggerEnter,
    this
  );
...
Brick.prototype._verifyTriggerActivationPossible = function () {
  if (this._pendingRigidbodyDisable) {
    if (this.entity.rigidbody && this.entity.rigidbody.enabled) {
      ...
      const velocityDamping = GameConfig.getAttribute(
        "gameplay",
        "brickLinearVelocityDamping"
      );
      ...
      const angularVelocityDamping = GameConfig.getAttribute(
        "gameplay",
        "brickAngularVelocityDamping"
      );
      ...
      if (
        this.entity.rigidbody.linearVelocity.length() <
          GameConfig.getAttribute("gameplay", "brickLinearVelocityThreshold") &&
        this.entity.rigidbody.angularVelocity.length() <
          GameConfig.getAttribute("gameplay", "brickAngularVelocityThreshold")
      ) {
...
Brick.prototype._verifyFallOutCondition = function () {
  if (
    this.entity.getPosition().y < GameConfig.getAttribute("gameplay", "groundY")
  ) {
  ...
  if (otherEntity.tags.has("stickman")) {
    ...
    if (
      ...
      (this.entity.color === stickman.color ||
        this.entity.color === this.app.colorsStorage.getDefaultColor())
    ) {
      if (this.entity._activationDelayedCall) {
        this.entity._activationDelayedCall.stop();
        this.entity._activationDelayedCall = null;
...
Stickman.prototype._initializeEntityProperties = function () {
  ...
  Object.defineProperty(this.entity, "color", {
    ...
    set(value) {
      this._color = value;
      this.stickmanModel.model.model.meshInstances.forEach(
        (mesh) =>
          (mesh.material = this._app.colorsStorage.getMaterial(this._color))
      );
      this.rigidbody.group = this._app.colorsStorage.getStickmanCollisionGroup(
        this._color
      );
    },
  });
  ...
  Object.defineProperty(this.entity, "invincible", {
    get() {
      ...
      if (this.floor && (this.floor.isStairs || this.floor.isFinishPlatform))
        return true;
    },
  });
  ...
  Object.defineProperty(this.entity, "slowdownFactor", {
    get() {
      return pc.math.clamp(
        1 -
          this.backpack.capacity *
            GameConfig.getAttribute("gameplay", "slowdownFactorPerBrick"),
        GameConfig.getAttribute("gameplay", "minRunningVelocityFactor"),
        1
      );
    },
  });
  ...
  Object.defineProperty(this.entity, "runningVelocity", {
    get() {
      return (
        GameConfig.getAttribute("movement", "velocity") * this.slowdownFactor
      );
    },
  });
...
Stickman.prototype._assignEntityMethods = function () {
  ...
  this.entity.pickupBrick = function (brick) {
    ...
    if (this.isPlayer) {
      this._app.fire(EventTypes.PLAY_SFX, [
        Constants.AUDIO.PICKUP_BRICK_1,
        Constants.AUDIO.PICKUP_BRICK_2,
      ]);
  ...
  this.entity.teleportTo = function (worldPosition) {
    if (this.rigidbody) {
      ...
      console.warn(
        "Can not teleport stickman to its target position: rigidbody is undefined",
        this
      );
    }
  }.bind(this.entity);
  ...
  this.entity.jumpTo = function (targetPosition) {
    if (this.rigidbody) {
      ...
      this._jumpTargetPosition = targetPosition
        .clone()
        .sub(this._groundPositionDisplacement)
        .add(new pc.Vec3(0, 0.05, 0));
      this._jumpDeltaPosition = new pc.Vec3(
        this._jumpTargetPosition.x - this._jumpStartPosition.x,
        this._jumpTargetPosition.y - this._jumpStartPosition.y,
        this._jumpTargetPosition.z - this._jumpStartPosition.z
      );
      this._jumpDuration =
        Utils.distanceXZ(this._jumpStartPosition, this._jumpTargetPosition) /
        GameConfig.getAttribute("jumper", "horizontalSpeed");
      this.jumping = true;
      ...
      console.warn(
        "Can not jump stickman to its target position: rigidbody is undefined",
        this
      );
    }
  }.bind(this.entity);
...
Stickman.prototype.updateJumpingProgress = function (dt) {
  if (this.entity.jumping) {
    this.entity._jumpProgress = pc.math.clamp(
      this.entity._jumpProgress + dt / this.entity._jumpDuration,
      0,
      1
    );

    const easingPower = GameConfig.getAttribute("jumper", "easingPower");
    const tweenedProgressValue = pc[
      GameConfig.getAttribute("jumper", "easing")
    ](this.entity._jumpProgress);
    const progressA = Math.pow(tweenedProgressValue, easingPower);
    const progressB = Math.pow(tweenedProgressValue, 1 / easingPower);
    const progress =
      progressA * (1 - this.entity._jumpProgress) +
      progressB * this.entity._jumpProgress;
    this.entity.setPosition(
      this.entity._jumpStartPosition.x +
        this.entity._jumpDeltaPosition.x * progress,
      this.entity._jumpStartPosition.y +
        this.entity._jumpDeltaPosition.y * progress +
        GameConfig.getAttribute("jumper", "jumpCurve").value(progress) *
          GameConfig.getAttribute("jumper", "jumpHeight"),
      this.entity._jumpStartPosition.z +
        this.entity._jumpDeltaPosition.z * progress
    );
...
Stickman.prototype.updateInvincibleStatus = function (dt) {
  this.entity.rigidbody.mask = this.entity.invincible
    ? this.app.colorsStorage.getStickmanInvincibleMask()
    : this.app.colorsStorage.getStickmanCollisionMask(this.entity.color);
};
...
Stickman.prototype.updateInvincibleCountdown = function (dt) {
  if (this.entity.invincibleCountdown > 0) {
    ...
    this.entity.invincibleCountdown = Math.max(
      this.entity.invincibleCountdown,
      0
    );
  }
};
...
Stickman.prototype._initializeColliders = function () {
  this.entity.collision.on("collisionstart", (result) => {
    if (
      this.entity.alive &&
      result.other.tags.has("stickman") &&
      result.other.alive
    ) {
      if (this.entity.backpack.capacity < result.other.backpack.capacity) {
        ...
        this.entity.fire(
          EventTypes.Stickman.COLLIDED_WITH_OTHER_STICKMAN,
          result.other,
          result
        );
      }
...
ColorsStorage.prototype.initializeAvailableColors = function () {
  ...
  if (this.availableSkins.length === 0) {
    ...
    console.error(
      "First skin in  GameConfig->skins should be named as 'default'"
    );
  }
};

ColorsStorage.prototype.prepareRoundColors = function (
  amount,
  playerColorName
) {
  if (amount > this.coloredSkins.length - 1) {
    console.error(
      `ColorsStorage:prepareColors - not enough colors is storage (${this.coloredSkins.length}/${amount})`
    );
  }

  const playerSkin = this.coloredSkins.find(
    (skin) => skin.name === playerColorName
  );
  const otherColorsSkins = this.coloredSkins.filter(
  ...
  if (amount === 1) {
    ...
    if (amount <= 4) {
      let shuffledColors = Utils.shuffle(
        otherColorsSkins.slice(0, 4).filter((color) => !!color)
      );
      this.currentRoundSkins = [
        playerSkin,
        ...shuffledColors.reverse().slice(1 - amount),
      ];
    } else {
      this.currentRoundSkins = [
        playerSkin,
        ...otherColorsSkins.reverse().slice(1 - amount),
      ];
    }
  }
...
ColorsStorage.prototype.getStickmanCollisionGroup = function (colorName) {
  const colorIndex = this.currentRoundSkins.findIndex(
    (skin) => skin.name === colorName
  );
...
ColorsStorage.prototype.getMaterial = function (colorName) {
  ...
  if (skin) {
    ...
    console.warn(
      `ColorsStorage.getMaterial(${colorName}): there is no such color in colors storage`
    );
    return null;
  }
};
...
StickmanMovement.prototype.initialize = function () {
  ...
  this.app.on(
    EventTypes.GameInput.KEYBOARD_STARTED,
    this._onKeyboardStarted,
    this
  );
  this.app.on(EventTypes.GameInput.KEYBOARD_ENDED, this._onKeyboardEnded, this);
  this.app.on(
    EventTypes.GameInput.KEYBOARD_UPDATED,
    this._onKeyboardUpdated,
    this
  );
  ...
  this.on("destroy", () => {
    ...
    this.app.off(
      EventTypes.GameInput.TOUCH_STARTED,
      this._onInputStarted,
      this
    );
    ...
    this.app.off(
      EventTypes.GameInput.KEYBOARD_ENDED,
      this._onKeyboardEnded,
      this
    );
    this.app.off(
      EventTypes.GameInput.KEYBOARD_UPDATED,
...
StickmanMovement.prototype.update = function (dt) {
  ...
  if (GameSession.getInstance().sessionActive) {
    /* update friction */
    this.entity.rigidbody.friction =
      this.entity.floor && this.entity.floor.isStairs ? 0.4 : 0.1;

    const linearVelocity = this.entity.rigidbody.linearVelocity;

    if (
      !this.app.applicationPaused &&
      this.entity.alive &&
      !this.entity.jumping &&
      !this.entity.finished &&
      !GameSession.getInstance().sessionPaused
    ) {
      if (this._isTouchActive) {
        ...
        linearVelocity.set(
          linearVelocity.x * velocityDamping,
          linearVelocity.y,
          linearVelocity.z * velocityDamping
        );
      }
    }
...
StickmanMovement.prototype._updateTouchMovement = function (linearVelocity) {
  ...
  const targetAngle = Utils.normalizeDegreesAngle(
    this.currentAzimuth * pc.math.RAD_TO_DEG
  );
...
StickmanMovement.prototype._updateMouseMovement = function (linearVelocity) {
  ...
  const stickmanPosition = this.entity.groundPosition.add(
    GameConfig.getAttribute("hierarchy", "Camera").cameraDisplacement
  );
  const stickmanScreenPos = GameConfig.getAttribute(
    "hierarchy",
    "Camera"
  ).camera.worldToScreen(stickmanPosition);

  this.currentAzimuth = Math.atan2(
    this._mousePosition.x - stickmanScreenPos.x,
    this._mousePosition.y - stickmanScreenPos.y
  );
...
StickmanMovement.prototype._updateKeyboardMovement = function (linearVelocity) {
  ...
  this.currentAngle = pc.math.lerpAngle(
    Utils.normalizeDegreesAngle(this.currentAngle),
    targetAngle,
    GameConfig.getAttribute("input", "keyboardSmootheningLerp")
  );
...
StickmanAnimator.prototype.initialize = function () {
  ...
  this.on("destroy", () => {
    ...
    this.entity.off(
      "status:finishedAtPlace",
      this.onFinishedStatusUpdated,
      this
    );
  });
};
...
StickmanAnimator.prototype.update = function (dt) {
  ...
  this.entity._currentAnimSpeed = this.entity.alive
    ? Math.pow(this.entity.slowdownFactor, 0.57)
    : 1;
...
Stairs.prototype.update = function (dt) {
  if (
    GameConfig.getAttribute("debug", "showBlockers") !== this._displayBlockers
  ) {
    this._displayBlockers = GameConfig.getAttribute("debug", "showBlockers");
    this.blockers.forEach(
      (blocker, key) =>
        (blocker.model.enabled = !!GameConfig.getAttribute(
          "debug",
          "showBlockers"
        ))
    );
...
Stairs.prototype._initializeEntityComponents = function () {
  ...
  if (!this.platformFrom)
    return console.error("platform from is not set on " + this.entity.path);
  if (!this.platformTo)
    return console.error("platform to is not set on " + this.entity.path);
  if (!this.stairsContainer)
    return console.error("StairsContainer is not set on " + this.entity.path);
  if (!this.blockersContainer)
    return console.error(
      "blockers container is not set on " + this.entity.path
    );
  if (!this.blockerTemplate)
    return console.error("blocker template is not set on " + this.entity.path);
...
Stairs.prototype._assignEntityMethods = function () {
  ...
  this.entity.getNextStair = function (stair) {
    ...
    return index >= 0 && index < this.stairs.length - 1
      ? this.stairs[index + 1]
      : null;
  }.bind(this.entity);
  ...
  this.entity.getPrevStair = function (stair) {
    const index = this.stairs.indexOf(stair);
    return index > 0 && index < this.stairs.length
      ? this.stairs[index - 1]
      : null;
  }.bind(this.entity);
  ...
  this.entity.getNumPaintedStairs = function (color) {
    return this.stairs.reduce(
      (sum, current) => sum + (current.color === color ? 1 : 0),
      0
    );
  }.bind(this.entity);
  ...
  this.entity.getTotalPaintedStairs = function () {
    return this.stairs.reduce(
      (sum, current) =>
        sum + (this._app.colorsStorage.isColored(current.color) ? 1 : 0),
      0
    );
  }.bind(this.entity);
...
Stairs.prototype._addEventListeners = function () {
  ...
  this.entity.on(
    EventTypes.Stairs.STICKMAN_ENTERED_STAIR,
    this.onStickmanEnteredStair,
    this
  );
  this.entity.on(
    EventTypes.Stairs.STICKMAN_LEAVED_STAIR,
    this.onStickmanLeavedStair,
    this
  );
  this.entity.on(
    EventTypes.Stairs.UPDATE_BLOCKER_POSITION,
    this.onUpdateBlockerPosition,
    this
  );
};
...
Stairs.prototype.onUpdateBlockerPosition = function (stickman) {
  ...
  if (!stair) {
    console.log(
      "onUpdateBlockerPosition: no current stair for stickman " + stickman.color
    );
    return;
  }
  const movementDirection = this.getStickmanMovementDirectionOnStairs(
    stickman,
    stair
  );
...
Stairs.prototype.onStickmanEnteredStair = function (stickman, stair) {
  ...
  const cheatEnabled =
    GameConfig.getAttribute("debug", "unlimitedBricks") &&
    stickman.tags.has("player-stickman");

  if (
    stair.stairs.isPassed(stickman.color) &&
    stair.isLast() &&
    !cheatEnabled
  ) {
    if (
      stickman.floor &&
      stickman.floor.isStairs &&
      stickman.floor === this.entity
    ) {
      this.disableBlocker(stickman.color);
      //already at stairs, no need to block him
    } else {
      console.log(
        "An attempt to step at already passed stairs by [" +
          stickman.color +
          "] has been prevented"
      );
  ...
  if (stair.color !== stickman.color) {
    if (stickman.backpack.capacity > 0 || cheatEnabled) {
      ...
      if (movementDirection === Constants.StairsMovementDirection.UP) {
        const nextUnpaintedStair = this.getNextUnpaintedStair(
          stair.color,
          stair.index + stickman.backpack.capacity
        );
...
Stairs.prototype.getStickmanMovementDirectionOnStairs = function (
  stickman,
  stair
) {
  ...
  return nextDot > prevDot
    ? Constants.StairsMovementDirection.UP
    : Constants.StairsMovementDirection.DOWN;
};
...
Stairs.prototype.updateBlockerPosition = function (stickman, blockedStair) {
  ...
  if (blockedStair) {
    blocker.setPosition(
      blockedStair.getPosition().clone().add(new pc.Vec3(0, 1.3, 0))
    );
...
Stairs.prototype._createBlockerFor = function (color) {
  ...
  blocker.rigidbody.mask =
    this.app.colorsStorage.getBlockerCollisionMask(color);
  blocker.model.model.meshInstances[0].material = this.app.colorsStorage
    .getMaterial(color)
    .clone();
  ...
  Object.defineProperty(this.entity, "color", {
    ...
    set(value) {
      ...
      if (prevColor === "default" && this._color !== "default" && !wasEnabled) {
        const appearingStartScale = GameConfig.getAttribute(
          "stair",
          "appearingStartScale"
        );
        this._initialModelScale =
          this._initialModelScale || this.getLocalScale().clone();
        this.setLocalScale(
          appearingStartScale,
          appearingStartScale,
          appearingStartScale
        ...
        this.tween(this.getLocalScale())
          .to(
            this._initialModelScale,
            GameConfig.getAttribute("stair", "appearingDuration"),
            pc[GameConfig.getAttribute("stair", "appearingEasing")]
          )
          .start();

        const appearingStartPosition = GameConfig.getAttribute(
          "stair",
          "appearingStartPosition"
        );
        if (appearingStartPosition.length() > 0) {
          this._initialPosition =
            this._initialPosition || this.getLocalPosition().clone();
          this.setLocalPosition(
            this._initialPosition.x + appearingStartPosition.x,
            this._initialPosition.y + appearingStartPosition.y,
            this._initialPosition.z + appearingStartPosition.z
          );
      ...
      if (this._color) {
        this.model.model.meshInstances.forEach(
          (mesh) =>
            (mesh.material = this._app.colorsStorage.getMaterial(this._color))
        );
      }
...
Platform.prototype._initializeEntityComponents = function () {
  ...
  if (!this.brickSpotsContainer)
    return console.error(
      "BrickSpots container is not set on " + this.entity.path
    );
  if (!this.spawnSpotsContainer)
    return console.error("SpawnSpots is not set on " + this.entity.path);
  if (this.spawnSpotsContainer.children.length === 0)
    return console.error(
      "There are not enough spawn spots on " + this.entity.path
    );
  ...
  /* Platform index */
  this.entity.platformIndex =
    this.entity.parent.children.indexOf(this.entity) + 1;
...
Platform.prototype._initializeEntityProperties = function () {
  ...
  Object.defineProperty(this.entity, "bricks", {
    get() {
      return this._brickSpots
        .filter((brickSpot) => brickSpot.hasBrick())
        .map((brickSpot) => brickSpot.brick);
    },
  });
...
Platform.prototype._assignEntityMethods = function () {
  ...
  this.entity.createBrickSpotAt = function (x, y, z) {
    const brickSpot = GameConfig.getAttribute(
      "templates",
      "brickSpot"
    ...
    if (!brickSpot.init)
      console.warn("brickSpot.init is not a function at ", brickSpot.path);
  ...
  this.entity.hasColoredBricks = function (color) {
    for (let spot of this.brickSpots)
      if (spot.hasBrick() && spot.brick.color === color) return true;
    return false;
  }.bind(this.entity);

  this.entity.getBrickSpotsWithColoredBricks = function (color) {
    return this.brickSpots.filter(
      (spot) => spot.hasBrick() && spot.brick.color === color
    );
  }.bind(this.entity);

  this.entity.getColoredBrickSpotsSkipingColor = function (skipColor) {
    return this.brickSpots.filter(
      (spot) => spot.hasBrick() && spot.brick.color !== skipColor
    );
  }.bind(this.entity);
  ...
  this.entity.getMaxPossibleBricksOfColor = function (color) {
    ...
    const aliveStickmansAmount =
      GameplayController.getInstance().entity.numStickmans;
    const divisor = Math.min(
      aliveStickmansAmount,
      Math.max(
        1 / GameConfig.getAttribute("gameplay", "limitBricksPerPlatform"),
        this.unlockStatuses.size,
        this.ingoingStairs.length
      )
    );
...
Platform.prototype._addEventListeners = function () {
  this.app.on(
    EventTypes.LEVEL.PREPARE_SPAWN_SPOTS,
    this.prepareSpawnSpots,
    this
  );
  this.app.on(
    EventTypes.STICKMAN_CHANGED_PLATFORM,
    this.onStickmanChangedPlatform,
    this
  );
  this.entity.on(
    EventTypes.Platform.BRICK_SPOT_CLEARED,
    this.onBrickSpotCleared,
    this
  );
};
...
Platform.prototype.prepareSpawnSpots = function () {
  ...
  availableSpots.forEach((spawnSpot) => {
    ...
    if (color) {
      ...
      spawnSpot.model.model.meshInstances[0].material =
        this.app.colorsStorage.getMaterial(color);
      spawnSpot.model.enabled = GameConfig.getAttribute(
        "debug",
        "showSpawnSpots"
      );
    } else {
...
BrickSpot.prototype.initialize = function () {
  this.entity.model.enabled = GameConfig.getAttribute(
    "debug",
    "showBrickSpots"
  );
...
Aiplayer.prototype.initialize = function () {
  ...
  this._stuckingCheckTimer = GameConfig.getAttribute(
    "AI",
    "stuckingPreventionCooldown"
  );
  this._droppedBricksContainer = GameConfig.getAttribute(
    "hierarchy",
    "DroppedBricks"
  );
  this._lastKnownPosition = this.entity.getPosition().clone();
  this._chanceToReactOnDroppedBrick = GameConfig.getAttribute(
    "AI",
    "chanceToReactOnDroppedBrick"
  );
  this._chanceToPickupDroppedBrick = GameConfig.getAttribute(
    "AI",
    "chanceToPickupDroppedBrickVsNormalBrick"
  );
...
Aiplayer.prototype.update = function (dt) {
  if (
    GameSession.getInstance().sessionActive &&
    !GameSession.getInstance().sessionPaused
  ) {
...
Aiplayer.prototype.verifyAndResolveStucking = function (dt) {
  ...
  if (this._stuckingCheckTimer <= 0) {
    ...
    if (
      ...
      this._lastKnownPosition.distance(this.entity.getPosition()) <
        GameConfig.getAttribute("AI", "maxDistanceToConsiderStucked")
    ) {
      famobi.log(
        "AI [" + this.entity.color + "] got stuck, finding another target..."
      );
      if (this.entity.floor && this.entity.floor.isStairs) {
        //if moving downwards, remove the blocker
        this.entity.floor.fire(
          EventTypes.Stairs.UPDATE_BLOCKER_POSITION,
          this.entity
        );
      }
...
Aiplayer.prototype._addEventListeners = function () {
  ...
  this.entity.on(
    EventTypes.Stickman.BRICK_PICKED_UP,
    this._onBrickPickedUp,
    this
  );
  this.entity.on(EventTypes.Stickman.OUT_OF_BRICKS, this._onOutOfBricks, this);
  this.entity.on(
    EventTypes.AI.TARGET_POSITION_REACHED,
...
Aiplayer.prototype._removeEventListeners = function () {
  ...
  this.app.off(
    EventTypes.GameSession.SESSION_STOPED,
    this.stopProcessing,
    this
  );
...
Aiplayer.prototype.startProcessing = function () {
  this.entity.delayedCall(pc.math.random(100, 600), () => {
    GameSession.getInstance()
      .waitUntilSessionResumed()
      .then(() => this.doNextDecision());
  });
};
...
Aiplayer.prototype.debugDrawPathTargets = function () {
  if (
    ...
    GameConfig.getAttribute("debug", "drawAITargets")
  ) {
    const material = this.app.colorsStorage.getMaterial(this.entity.color);
    this._pathStartColor.set(
      material.diffuse.r,
      material.diffuse.g,
      material.diffuse.b,
      0.8
    );
    this._pathEndColor.set(
      material.diffuse.r,
      material.diffuse.g,
      material.diffuse.b,
      0.8
    );
    let startPosition = this.entity
      .getPosition()
      .add(this._debugPathStartPositionDisplacement);
    for (let pathPoint of this.currentPath) {
      this.app.renderLine(
        startPosition,
        pathPoint,
        this._pathStartColor,
        this._pathEndColor
      );
...
Aiplayer.prototype._moveToPosition = function (position) {
  if (position) {
    const positionDisplacement =
      GameConfig.getAttribute("AI", "targetDistanceThresholdH") / 4;
    position = position
      .clone()
      .add(
        new pc.Vec3(
          pc.math.random(-positionDisplacement, positionDisplacement),
          0,
          pc.math.random(-positionDisplacement, positionDisplacement)
        )
      );
...
Aiplayer.prototype.attackStrongestBot = function (strongestBot) {
  ...
  this.entity.fire(
    EventTypes.AI.SET_TARGET_POSITION,
    strongestBot.groundPosition
  );
  console.log(
    this.entity.color + " attacks strongest bot " + strongestBot.color
  );
...
Aiplayer.prototype.moveToNextBrick = function () {
  ...
  if (this.entity.floor && this.entity.floor.isPlatform) {
    ...
    if (closestNode) {
      const accessibleNodes = this.entity.floor.findAccessibleColoredNodes(
        closestNode,
        this.entity.color,
        true
      );
      if (accessibleNodes.length > 0) {
        accessibleNodes.sort(
          (a, b) =>
            Utils.distanceSq(stickmanPosition, a.getPosition()) -
            Utils.distanceSq(stickmanPosition, b.getPosition())
        );
        const wantedNode = accessibleNodes[0]; //Utils.randomWeightedChoise(accessibleNodes, 12);
        const path = this.entity.floor.getPath(
          closestNode,
          wantedNode,
          true,
          true
...
Aiplayer.prototype.moveToClosestDroppedBrick = function () {
  this._allowedRayThroughEntityNames = this._allowedRayThroughEntityNames || [
    "BrickTrigger",
  ];
  this._groundYDisplacement =
    this._groundYDisplacement || new pc.Vec3(0, 0.09, 0);
  const stickmanGroundPosition = this.entity.groundPosition.add(
    this._groundYDisplacement
  );
  if (this.entity.floor && this.entity.floor.isPlatform) {
    ...
    for (let brick of this._droppedBricksContainer.children) {
      ...
      const distanceXZ = Utils.distanceXZ(
        stickmanGroundPosition,
        brickPosition
      );
      ...
      if (
        ...
        distanceXZ > 1 &&
        distanceXZ <
          GameConfig.getAttribute("AI", "droppedBrickVisionDistance") &&
        this.isAccessibleWithRaycast(
          stickmanGroundPosition,
          brickPosition,
          this._allowedRayThroughEntityNames
        )
      ) {
        //
    ...
    if (closestDroppedBrick) {
      ...
      this.startFollowingPath(path.map((node) => node.getPosition()));
      this.log(
        "Moving to closest dropped brick ",
        closestDroppedBrick,
        closestDroppedBrick.getPosition()
      );
      return true;
    }
  }
  ...
  if (this.entity.floor && this.entity.floor.isPlatform) {
    ...
    if (closestNode) {
      const accessibleNodes =
        this.entity.floor.findAccessibleExitNodes(closestNode);
      if (accessibleNodes.length > 0) {
        ...
        famobi.log(
          "[moveToAppropriateStairs] " +
            this.entity.color +
            ": no accessible exit nodes found"
        );
      }
    } else {
    ...
    famobi.log(
      "[moveToAppropriateStairs] " + this.entity.color + ": not on platform"
    );
  }
...
Aiplayer.prototype.moveToClosestJumper = function () {
  ...
  const stickmanPosition = this.entity.getPosition();
  const jumperNodes = (this.entity.floor.jumpers || [])
    .slice()
    .filter((jumper) =>
      this.isAccessibleWithRaycast(
        stickmanPosition,
    ...
    .sort(
      (a, b) =>
        stickmanPosition.distance(a.getPosition()) -
        stickmanPosition.distance(b.getPosition())
    );
  if (jumperNodes.length > 0) {
    ...
    famobi.log(
      "[moveToClosestJumper] " + this.entity.color + ": no closest exit node"
    );
  }
  return false;
};
...
Aiplayer.prototype._selectAppropriateStairsNode = function (accessibleNodes) {
  ...
  if (
    accessibleNodes.every(
      (node) => node.stairs.getNumPaintedStairs(this.entity.color) === 0
    )
  ) {
    const emptyStairsNodes = accessibleNodes.filter(
      (node) => node.stairs.getTotalPaintedStairs() === 0
    );
  ...
  const paintedStairsNodes = accessibleNodes.filter(
    (node) => node.stairs.getNumPaintedStairs(this.entity.color) > 0
  );
  const unpaintedStairsNodes = accessibleNodes.filter(
    (node) => node.stairs.getNumPaintedStairs(this.entity.color) === 0
  );
  let targetNodes =
    paintedStairsNodes.length > 0 ? paintedStairsNodes : unpaintedStairsNodes;

  if (paintedStairsNodes.length > 0) {
    //sort stairs by amount of painted stairs & distance
    Utils.shuffle(paintedStairsNodes).sort((a, b) => {
      if (
        a.stairs.getNumPaintedStairs(this.entity.color) >
        b.stairs.getNumPaintedStairs(this.entity.color)
      )
        return -1;
      if (
        a.stairs.getNumPaintedStairs(this.entity.color) <
    ...
    Utils.shuffle(unpaintedStairsNodes).sort((a, b) => {
      if (a.stairs.getTotalPaintedStairs() < a.stairs.getTotalPaintedStairs())
        return -1;
      if (a.stairs.getTotalPaintedStairs() > a.stairs.getTotalPaintedStairs())
        return 1;
      return a._accumulatedDistance - b._accumulatedDistance;
...
Aiplayer.prototype.doNextDecision = function () {
  if (this.entity.floor) {
    ...
    console.warn(
      "AI [" +
        this.entity.color +
        "] can not make a decision, current floor is not set"
    );
    this._moveToPosition(null);
  }
};
...
Aiplayer.prototype._doNextDecisionOnPlatform = function () {
  const hasColoredBricksOnThePlatform = this.entity.floor.hasColoredBricks(
    this.entity.color
  );
  const isPlayerAhead = AIManager.getInstance().isPlayerAhead(this.entity);
  const isStrongestBot = AIManager.getInstance().isStrongestBotOnHisPlatform(
    this.entity
  );
  const strongestBot = AIManager.getInstance().getStrongestBot(
    this.entity.floor
  );
  const platformBots = AIManager.getInstance().getStrongestBotsOfPlatform(
    this.entity.floor
  );
  ...
  if (hasColoredBricksOnThePlatform) {
    /*if(this.entity.backpack.capacity > 2 && platformBots.length > 1 && Utils.distanceXZ(this.entity.getPosition(), strongestBot.getPosition()) < 20 && Math.random() < 0.5 && isPlayerAhead && !isStrongestBot) {
      ...
      (AIManager.getInstance().isStrongestBotOnHisPlatform(this.entity) ||
        this.diceRoll(this._chanceToPickupDroppedBrick)) &&
      this.moveToClosestDroppedBrick()
    ) {
      this.log("moving to closest dropped brick");
    } else if (
      this.entity.backpack.capacity > 0 &&
      Math.random() <
        this.entity.backpack.capacity *
          GameConfig.getAttribute("AI", "goToStairsChancePerEachBrick")
    ) {
    ...
    if (this.moveToClosestDroppedBrick()) {
      this.log(
        "moving to closest dropped brick 'cause nothing else left to do :( "
      );
...
Aiplayer.prototype._doNextDecisionOnStairs = function () {
  const closestNode = this.entity.floor.getClosestNode(
    this.entity.getPosition()
  );
  const isExitOpened = this.entity.floor.isExitOpenedForColor(
    closestNode,
    this.entity.color
  );
...
/* Raycasting */
Aiplayer.prototype.isAccessibleWithRaycast = function (
  positionA,
  positionB,
  allowedNames
) {
...
Aimovement.prototype.verifyTargetReached = function () {
  if (this.isAbleToMove()) {
    const stickmanPosition = this.entity.getPosition();
    const horizontalDistanceToTarget = Utils.distanceXZ(
      stickmanPosition,
      this.targetPosition
    ); // this.entity.getPosition().distance(this.targetPosition);
    const verticalDistanceToTarget = Utils.distanceY(
      stickmanPosition,
      this.targetPosition
    );
    ...
    if (
      horizontalDistanceToTarget <
        GameConfig.getAttribute("AI", "targetDistanceThresholdH") &&
      verticalDistanceToTarget <
        GameConfig.getAttribute(
          "AI",
          "targetDistanceThresholdV"
        ) /* || distanceIncreased*/
    ) {
...
Aimovement.prototype.moveToTarget = function (dt) {
  if (this.isAbleToMove()) {
    ...
    const currentAzimuth = Math.atan2(
      this.targetPosition.x - stickmanPosition.x,
      this.targetPosition.z - stickmanPosition.z
    );

    const targetAngle = Utils.normalizeDegreesAngle(
      currentAzimuth * pc.math.RAD_TO_DEG
    );

    /* smoother turning V1 */
    const maxFrameDeltaAngle =
      (this.entity.floor && this.entity.floor.isStairs
        ? GameConfig.getAttribute("AI", "stairsTurnSpeed")
        : GameConfig.getAttribute("AI", "turnSpeed")) * dt;
    let shortestAngle = Utils.shortestAngleBetween(
      Utils.normalizeDegreesAngle(this.currentAngle),
      targetAngle
    );
    ...
    const dx =
      Math.sin(targetAngle * pc.math.DEG_TO_RAD) *
      this.entity.runningVelocity *
      GameConfig.getAttribute("AI", "velocityScale");
    const dz =
      Math.cos(targetAngle * pc.math.DEG_TO_RAD) *
      this.entity.runningVelocity *
      GameConfig.getAttribute("AI", "velocityScale");
    ...
    if (AIManager.getInstance().isPlayerAhead(this.entity)) {
      ...
      if (strongestBots.length > 1) {
        aiSpeedBonus =
          (1 +
            ((strongestBots.length - strongestBots.indexOf(this.entity)) /
              strongestBots.length) *
              (speedupValue - 1)) /
          this.entity.slowdownFactor;
      } else {
...
Aimovement.prototype.debugDrawTargetPosition = function () {
  if (
    ...
    this._targetRendererColor =
      this._targetRendererColor || new pc.Color(0, 0, 0, 0.75);
    const material = this.app.colorsStorage.getMaterial(this.entity.color);
    this.app.renderLine(
      this.entity.getPosition(),
      this.targetPosition,
...
DatGui.prototype.initialize = function () {
  ...
  const clearLevelsTest = () =>
    this.app.root.findByName("LevelsTest") &&
    this.app.root.findByName("LevelsTest").destroy();
...
DatGui.prototype.setLevel = function (key, value) {
  LevelController.getInstance().setCurrentLevelIndex(
    LevelController.getInstance().getLevelIndexByName(value)
  );
...
DatGui.prototype.addStringOption = function (
  folder,
  optionHolder,
  optionKey,
  listener,
  ...
  return folder
    ...
    .onFinishChange((value) =>
      listener.call(listenerContext, optionKey, value)
    );
};
...
FinishPlatform.prototype._initializeEntityComponents = function () {
  ...
  if (!this.finishSpotsContainer)
    return console.error(
      "FinishSpotsContainer is not set on finish platform " + this.entity.path
    );
  if (this.finishSpotsContainer.children.length < 3)
    return console.error(
      "There are not enough finish spots on " + this.entity.path
    );
...
FinishPlatform.prototype._assignEntityMethods = function () {
  ...
  this.entity.placeWinnerStickman = function (stickman) {
    if (this.winners.length < 3) {
      ...
      console.error(
        "Stickman can not finish cause all 3 winners are already defined"
      );
      return 0;
    }
  }.bind(this.entity);
...
FinishPlatform.prototype.prepareFinishSpots = function () {
  ...
  this.entity.finishSpots.forEach((finishSpot) => {
    finishSpot.model.enabled = GameConfig.getAttribute(
      "debug",
      "showSpawnSpots"
    );
  });
...
BasicButton.assignAction = function (button, handler, handlerContext) {
  if (pc.app.touch) {
    button.element.on(
      this.triggerEvent === "down" ? "touchstart" : "touchend",
      handler,
      handlerContext
    );
  }
  if (pc.app.mouse) {
    button.element.on(
      this.triggerEvent === "down" ? "mousedown" : "mouseup",
      handler,
      handlerContext
    );
  }
...
UIController.getInstance = function () {
  if (!UIController._instance)
    console.error("UIController is not initialized yet");
  ...
  this.entity.children.forEach((child) => {
    if (child.tags.has("ui-window")) {
      ...
      famobi.log(
        "UI location " +
          child.name +
          " should probably have a ui-window tag in order to be properly initialized"
      );
    }
  });
};

UIController.prototype.showWindow = function (
  windowName,
  hideOthers = true,
  windowToOpenOnExit = null
) {
  const windowEntity = this.entity.children.find(
...
UIController.prototype.showWindowImmediately = function (
  windowName,
  hideOthers = true,
  windowToOpenOnExit = null
) {
...
UIController.prototype.hideAll = async function (...skipWindows) {
  const windowToHidePromises = this.entity.children
    .filter(
      (windowEntity) =>
        windowEntity.tags.has("ui-window") &&
        skipWindows.indexOf(windowEntity) === -1
    )
...
UIController.prototype._showWindow = async function (
  windowEntity,
  windowToOpenOnExit = null
) {
...
class BaseWindow extends pc.ScriptType {
  ...
  appearElements() {
    ...
    const completionPromises = elements.map(
      (element) =>
...
class SettingsWindow extends CloseableWindow {
  ...
  /* private */
  updateSoundButtons() {
    this.sfxButton.findByName("IconMuted").enabled =
      !SoundController.sfxEnabled;
    this.sfxVolumeBar.fire("setSliderValue", SoundController.sfxVolume);
  }

  updateMusicButtons() {
    this.musicButton.findByName("IconMuted").enabled =
      !SoundController.musicEnabled;
  ...
  onHomeClicked() {
    ...
    window.famobi_analytics
      .trackEvent("EVENT_LEVELFAIL", {
        levelName: `${
          LevelController.getInstance().getCurrentLevelIndex() + 1
        }`,
        reason: "quit",
      })
      .then(() => {
...
class SkinsWindow extends CloseableWindow {
  ...
  _initComponents() {
    ...
    /* texts */
    this.extraCoinsRewardAmount = this.entity.findByName(
      "ExtraCoinsRewardAmount"
    );
    this.extraCoinsRewardAmount.element.text =
      "+" +
      GameConfig.getAttribute(
        "gameplay",
        "skinsExtraCoinsForWatchingAVideoAmount"
      );

    /* event listeners */
    BasicButton.assignAction(
      this.characterSkinsButton,
      this.showCharactersSkinsPanel,
      this
    );
    BasicButton.assignAction(
      this.brickSkinsButton,
      this.showBricksSkinsPanel,
      this
    );
    BasicButton.assignAction(
      this.colorSkinsButton,
      this.showColorsSkinsPanel,
      this
    );
  ...
  _onShow() {
    ...
    this.extraCoinsButton.on(
      EventTypes.UI_ELEMENT.APPEARED,
      this._enableExtraOffer,
      this
    );
  }
  ...
  showCharactersSkinsPanel() {
    [this.panelBricks, this.panelColors, this.panelCharacters].forEach((c) =>
      c.reparent(this.skinsContainer)
    );
  }

  showBricksSkinsPanel() {
    [this.panelColors, this.panelCharacters, this.panelBricks].forEach((c) =>
      c.reparent(this.skinsContainer)
    );
  }

  showColorsSkinsPanel() {
    [this.panelBricks, this.panelCharacters, this.panelColors].forEach((c) =>
      c.reparent(this.skinsContainer)
    );
  }
...
class LevelsWindow extends CloseableWindow {
  ...
  initializeLevelButtons() {
    ...
    this.levelButtonsGroup.element.height =
      (this.levelButtonsGroup.children.length + 1) * (280 + 20) + 40;
  ...
  scrollToLevel(levelIndex) {
    const screenUpscaleFactor =
      this.app.graphicsDevice.height /
      this.app.graphicsDevice.width /
      (1280 / 720);
    const deadSpacing = pc.math.clamp(
      Math.floor((1280 * pc.math.clamp(screenUpscaleFactor, 1, 2)) / 350),
      2,
      LevelController.getInstance().getTotalLevels() / 2 - 1
    );
    if (levelIndex < deadSpacing) {
      this.scrollBar.scrollbar.value = 0;
    } else if (
      levelIndex >=
      LevelController.getInstance().getTotalLevels() - deadSpacing
    ) {
      this.scrollBar.scrollbar.value = 1;
    } else {
      this.scrollBar.scrollbar.value = pc.math.clamp(
        (levelIndex + 0.5 - deadSpacing) /
          (LevelController.getInstance().getTotalLevels() - 2 * deadSpacing),
        0,
        1
      );
    }
...
class StartWindow extends BaseWindow {
  ...
  /* event handlers */
  onSettingsClicked() {
    UIController.getInstance().showWindow(
      Constants.UI_LOCATIONS.SETTINGS,
      true,
      Constants.UI_LOCATIONS.START
    );
  }

  onLevelsClicked() {
    UIController.getInstance().showWindow(
      Constants.UI_LOCATIONS.LEVELS,
      true,
      Constants.UI_LOCATIONS.START
    );
  }
  ...
  onSpecialOfferClicked() {
    const startGame = (grantReward) => {
      ...
      if (grantReward)
        this.app.fire(EventTypes.AWARD_PLAYER_STICKMAN_WITH_BRICKS, 10);
    };
...
StickmanFloorDetector.prototype.detectFloor = function () {
  ...
  for (let result of results) {
    if (
      result.entity &&
      result.entity.tags.has("platform-collider", "stairs-collider")
    ) {
...
class VictoryWindow extends BaseWindow {
  ...
  _initComponents() {
    ...
    this.extraBricksRewardAmount = this.entity.findByName(
      "ExtraBricksRewardAmount"
    );
    this.watchVideoRewardAmount = this.entity.findByName(
      "WatchVideoRewardAmount"
    );
    this.continueRewardAmount = this.entity.findByName("ContinueRewardAmount");
    this.droppedCoinsContainer = this.entity.findByName(
      "DroppedCoinsContainer"
    );
    ...
    /* event listeners */
    BasicButton.assignAction(
      this.regularRewardButton,
      this.onRegularRewardClicked,
      this
    );
    BasicButton.assignAction(
  ...
  _onShow() {
    ...
    /* update level texts */
    this.levelNumberText.element.text =
      "" + (LevelController.getInstance().getCurrentLevelIndex() + 1);

    this.levelPassTime.element.text =
      "" + Utils.humanizeTime(LevelStatsManager.getInstance().getLevelTime());

    this.extraBricks.element.text =
      "" + LevelStatsManager.getInstance().getExtraBricks();

    this._levelReward = LevelStatsManager.getInstance().getLevelReward();
    this._levelExtraReward = Math.max(
      this._levelReward *
        GameConfig.getAttribute(
          "gameplay",
          "victoryWatchVideoRewardMultiplier"
        ),
      50
    );
    ...
    /* set & tween texts */
    const levelTimeReward =
      LevelStatsManager.getInstance().getLevelTimeReward();
    const levelTimeTweenDuration = pc.math.clamp(
      levelTimeReward / 100,
      0.15,
      0.75
    );
    if (levelTimeReward > 0) {
      ...
      Utils.tweenText(
        this.passTimeRewardAmount,
        0,
        levelTimeReward,
        levelTimeTweenDuration,
        0.4,
        pc.Linear,
        true,
        "+"
      ...
      this.passTimeRewardAmount.element.text =
        "+" + LevelStatsManager.getInstance().getLevelTimeReward();
    }

    const extraBricksReward =
      LevelStatsManager.getInstance().getExtraBricksReward();
    if (extraBricksReward > 0) {
      this.extraBricksRewardAmount.element.text = "+" + extraBricksReward;
      Utils.tweenText(
        this.extraBricksRewardAmount,
        0,
        extraBricksReward,
        pc.math.clamp(extraBricksReward / 50, 0.15, 0.75),
        0.4 + levelTimeTweenDuration + 0.075,
        pc.Linear,
        true,
        "+"
      );
      ...
      Promise.all([
        ...
        window.famobi_analytics.trackEvent("EVENT_TOTALSCORE", {
          totalScore: Math.floor(
            LevelStatsManager.getInstance().getLevelScore()
          ),
        }),
  ...
  _showButtons() {
    if (Apicontroller.hasRewardedVideo()) {
      ...
      this.entity.delayedCall(350, () =>
        CoinsStorage.getInstance().addCoins(this._levelReward)
      );
  ...
  onDoubleRewardClicked() {
    const doRewardPlayer = (grantReward) => {
      if (grantReward) {
        this.app.fire(EventTypes.PLAY_SFX, Constants.AUDIO.MULTIPLE_COINS);
        this.entity.delayedCall(750, () =>
          CoinsStorage.getInstance().addCoins(this._levelExtraReward)
        );
        this.droppedCoinsContainer.setLocalPosition(
          this.doubleRewardButton.getLocalPosition()
        );
  ...
  onRegularRewardClicked() {
    ...
    this.droppedCoinsContainer.setLocalPosition(
      this.regularRewardButton.getLocalPosition()
    );
  ...
  playDroppedCoinsAnimation(amount) {
    ...
    for (let i = 0; i < amount; i++) {
      ...
      coin.setLocalPosition(
        pc.math.random(-4 * amount, 4 * amount),
        pc.math.random(-2 * amount, 2 * amount),
        0
      );
  ...
  if (this.useDevicePixelRatio) {
    ...
    if (pc.platform.desktop && this.app.graphicsDevice.maxPixelRatio > 1.99) {
      this.app.graphicsDevice.maxPixelRatio = pc.math.clamp(
        1 + (this.app.graphicsDevice.maxPixelRatio - 1) / 2,
        1,
...
PlatformNavigationGraph.prototype.initialize = function () {
  ...
  this.app.on(
    EventTypes.LEVEL.BUILD_NAVIGATION_GRAPH,
...
PlatformNavigationGraph.prototype._assignEntityMethods = function () {
  ...
  this.entity.findAccessibleColoredNodes = function (
    nodeFrom,
    color,
    avoidExitNodes
  ) {
    return scriptContext.findAccessibleColoredNodes(
      nodeFrom,
      color,
      avoidExitNodes
    );
...
PlatformNavigationGraph.prototype.update = function (dt) {
  if (GameConfig.getAttribute("debug", "drawSpotsConnections")) {
    this.entity.brickSpots.forEach((bs) => {
      if (bs.neighbors) {
        bs.neighbors.forEach((n) =>
          this.app.renderLine(
            bs.getPosition(),
            n.getPosition(),
            new pc.Color(0, 0, 0, 0.15)
          )
        );
      }
    });
...
PlatformNavigationGraph.prototype._buildNavigationGraph = function () {
  ...
  this.entity._exitNodes = this.entity.outgoingStairs.map((stairs) =>
    stairs.getFirstStair()
  );
  if (this.entity.exitNodes.length === 0)
    famobi.log(
      "Warning: the platform " + this.entity.name + " does not have exit nodes "
    );

  this.entity._entranceNodes = this.entity.ingoingStairs.map((stairs) =>
    stairs.getLastStair()
  );
  if (this.entity.entranceNodes.length === 0)
    famobi.log(
      "Warning: the platform " +
        this.entity.name +
        " does not have entrance nodes "
    );

  const exitEntranceNodes = [
    ...this.entity.exitNodes,
    ...this.entity.entranceNodes,
  ];

  const connectCustomNode = (node, nodePosition, stair, stairPosition) => {
    if (
      nodePosition.distance(stairPosition) < this.maxSpotStairConnectionLength
    ) {
  ...
  for (let i = 0; i < this.entity.brickSpots.length; i++) {
    const spotA = this.entity.brickSpots[i],
      positionA = spotA.getPosition();

    /* connections between nodes */
    for (let j = i + 1; j < this.entity.brickSpots.length; j++) {
      const spotB = this.entity.brickSpots[j],
        positionB = spotB.getPosition();
    ...
    exitEntranceNodes.forEach((stair) => {
      ...
      if (
        positionA.distance(stairPosition) < this.maxSpotStairConnectionLength
      ) {
...
PlatformNavigationGraph.prototype.findAccessibleColoredNodes = function (
  nodeFrom,
  color,
  avoidExitNodes
) {
  ...
  return this.entity.brickSpots.filter(
    (brickSpot) =>
      brickSpot._accumulatedDistance >= 0 &&
      brickSpot.hasBrick() &&
      brickSpot.brick.color === color
  );
};

PlatformNavigationGraph.prototype.findAccessibleExitNodes = function (
  nodeFrom
) {
  this._resetSpotsMarks();
  this._markSpots([nodeFrom], null, 0);
  return Utils.shuffle(
    this.entity.exitNodes.filter(
      (exitNode) => exitNode._accumulatedDistance >= 0
    )
  );
};

PlatformNavigationGraph.prototype.getPath = function (
  nodeFrom,
  nodeTo,
  skipFirstNode,
  avoidExitNodes
  ...
  return this._backPropagatePath(
    nodeFrom,
    nodeTo,
    skipFirstNode,
    avoidExitNodes
...
PlatformNavigationGraph.prototype._markSpots = function (
  ...
  for (let n of currentRoundNeighbors) {
    ...
    if (n._accumulatedDistance < 0 || !n._accumulatedDistance)
      n._accumulatedDistance = 0;
    ...
    for (let nn of n.neighbors) {
      ...
      if (
        avoidExitNodes &&
        nn !== nodeTo &&
        this.entity.exitNodes.indexOf(nn) !== -1
      ) {
...
PlatformNavigationGraph.prototype._backPropagatePath = function (
  nodeFrom,
  nodeTo,
  skipFirstNode,
  avoidExitNodes
  ...
  do {
    ...
    for (let neighbor of currentNode.neighbors) {
      ...
      if (
        neighbor._accumulatedDistance >= 0 &&
        neighbor._accumulatedDistance < minDistance
      ) {
...
PlatformNavigationGraph.prototype.isAccessible = function (
  positionA,
  positionB
) {
  // return true;
  ...
  return this.entity.skipZonesAABBs.every(
    (skipZoneAABB) =>
      !(
        skipZoneAABB.intersectsRay(this._testRayAB) &&
        skipZoneAABB.intersectsRay(this._testRayBA)
      )
  );
};

PlatformNavigationGraph.prototype.isAccessibleWithRaycast = function (
  positionA,
  positionB
) {
  this._rayStartPos = this._rayStartPos || new pc.Vec3();
...
StairsNavigationGraph.prototype.update = function (dt) {
  if (GameConfig.getAttribute("debug", "drawSpotsConnections")) {
    this.entity.stairs.forEach((stair) => {
      if (stair.neighbors) {
        stair.neighbors.forEach((n) =>
          this.app.renderLine(
            stair.getPosition(),
            n.getPosition(),
            new pc.Color(0, 0, 0, 0.25)
          )
        );
      }
    });
...
StairsNavigationGraph.prototype._buildNavigationGraph = function () {
  ...
  this.entity._entranceNodes = Array.from(
    this.entity.getFirstStair().neighbors
  ).filter((neighbor) => !neighbor.isStair);
  if (this.entity.platformTo.isFinishPlatform) {
    this.entity._exitNodes = [this.entity.platformTo.centerNode];
  } else {
    this.entity._exitNodes = Array.from(
      this.entity.getLastStair().neighbors
    ).filter((neighbor) => !neighbor.isStair);
  }
...
StairsNavigationGraph.prototype.getPath = function (
  nodeFrom,
  nodeTo,
  skipFirstNode
) {
  if (nodeTo === nodeFrom) {
    console.warn(
      "StairsNavigationGraph:getPath - from and to nodes are the same"
    );
    return skipFirstNode ? [nodeTo] : [nodeFrom, nodeTo];
  }
  if (this.entity.exitNodes.indexOf(nodeTo) !== -1) {
    const nodesList = this.entity.stairs.filter(
      (stair) => stair.index >= nodeFrom.index
    );
  ...
  if (this.entity.entranceNodes.indexOf(nodeTo) !== -1) {
    const nodesList = this.entity.stairs
      .filter((stair) => stair.index <= nodeFrom.index)
      .reverse();
    nodesList.push(nodeTo);
    return skipFirstNode ? nodesList.slice(1) : nodesList;
  }
  ...
  const nodesList = this.entity.stairs.filter(
    (stair, index) =>
      stair.index >= Math.min(nodeFrom.index, nodeTo.index) &&
      stair.index <= Math.max(nodeFrom.index, nodeTo.index)
  );
...
pc.script.createLoadingScreen(function (app) {
  var showSplash = function () {
    ...
    [
      ...
      wrapper.addEventListener(evtKey, function (e) {
        e.stopPropagation();
        e.preventDefault();
      })
    );
  ...
  var injectForcedModeProperties = function () {
    ...
    if (forcedModeProperties.state.level > 0) {
      ...
      LevelController.CURRENT_LEVEL_INDEX =
        forcedModeProperties.state.level - 1;
      LevelController.MAX_UNLOCKED_LEVEL_INDEX = Math.max(
        LevelController.CURRENT_LEVEL_INDEX,
        LevelController.MAX_UNLOCKED_LEVEL_INDEX
      );
    }
        ...
        doAPIHandshake(() => {
          ...
          if (isForcedMode()) {
            ...
            if (forcedModeProperties.state.initial_bricks > 0) {
              app.fire(
                EventTypes.AWARD_PLAYER_STICKMAN_WITH_BRICKS,
                forcedModeProperties.state.initial_bricks
              );
            }
          }
        }),
...
GameplayController.getInstance = function () {
  if (!GameplayController._instance)
    console.error("GameplayController is not initialized yet");
...
GameplayController.prototype.initialize = function () {
  ...
  this.entity.levelContainer = GameConfig.getAttribute(
    "hierarchy",
    "LevelContainer"
  );
  ...
  this.app.on(
    EventTypes.RELOAD_LEVEL_AND_SHOW_MENU,
    this._reloadLevelAndShowMenu,
    this
  );
...
GameplayController.prototype._loadInitialLevel = function () {
  if (this.testLevel) {
    const levelIndex = LevelController.getInstance().getLevelIndexByName(
      this.testLevel.name
    );
    if (levelIndex === -1) {
      alert(
        "Please add the level prefab you want to test in Gameplay>LevelController=>levelTemplates script attributes"
      );
    } else {
      console.warn(
        "Loading test level " +
...
GameplayController.prototype._loadLevel = function (skipTitleScreen) {
  LevelController.getInstance()
    ...
    .then(() => {
      famobi.log(
        "Level " +
          LevelController.getInstance().getCurrentLevelName() +
          " has been loaded"
      );
    ...
    .catch((errorMessage) =>
      console.error("Level creation failed: ", errorMessage)
    );
};
...
GameplayController.prototype._spawnStickmans = function () {
  ...
  for (
    let i = this.entity.stickmansContainer.children.length - 1;
    i > -1;
    i--
  ) {
  ...
  const aiColors = Utils.shuffle(
    this.app.colorsStorage.getColorsList().filter((c) => c != playerColor)
  );
  const names = GameConfig.getAttribute("AI", "nicknames").slice();
  const totalEnemies = this.entity.level.totalEnemies;

  //player stickman
  this.entity.playerStickman = GameConfig.getAttribute(
    "templates",
    "stickman"
  ).resource.instantiate();
  ...
  this.entity.playerStickman.teleportTo(
    this.entity.level.firstPlatform.getSpawnPosition(
      this.entity.playerStickman.color
    )
  );
  this.app.fire(
    EventTypes.SET_GAMEPLAY_CAMERA_TARGET,
    this.entity.playerStickman
  );
  ...
  for (let i = 0; i < totalEnemies; i++) {
    ...
    const stickmanAI = GameConfig.getAttribute(
      "templates",
      "stickmanAI"
    ).resource.instantiate();
    ...
    stickmanAI.teleportTo(
      this.entity.level.firstPlatform.getSpawnPosition(stickmanAI.color)
    );
    this.entity.stickmansAI.push(stickmanAI);
  }
};

GameplayController.prototype._onStickmanFinished = function (
  stickman,
  finishPlatform
) {
  ...
  const winnerPlace =
    this.entity.level.finishPlatform.placeWinnerStickman(stickman);
  ...
  if (stickman === this.entity.playerStickman) {
    ...
    if (winnerPlace === 1) {
      const secondPlace = this.entity.level.finishPlatform.placeWinnerStickman(
        this.entity.playerStickman
      );
...
LevelController.getInstance = function () {
  if (!LevelController._instance)
    console.error("LevelController is not initialized yet");
...
LevelController.prototype.initialize = function () {
  ...
  this.app.on(
    EventTypes.ON_PLAYER_SKIN_CHANGED,
    this.onPlayerSkinChanged,
    this
  );
  this.app.on(
    EventTypes.ON_PLAYER_COLOR_CHANGED,
  ...
  if (!LevelController.levelSetViaForcedMode) {
    this.setCurrentLevelIndex(
      LocalStorageController.getSavedValue(Constants.Storage.CURRENT_LEVEL)
    );
    this.setMaxUnlockedLevelIndex(
      LocalStorageController.getSavedValue(Constants.Storage.MAX_UNLOCKED_LEVEL)
    );
  }
};
...
LevelController.prototype.onPlayerSkinChanged = function () {
  if (this.entity.playerStickman) {
    this.entity.playerStickman.setAccessory(
      SkinsManager.getInstance().getSelectedCharacterSkin()
    );
  }
};
...
LevelController.prototype.getMaxUnlockedLevelIndex = function () {
  return GameConfig.getAttribute("debug", "unlockAllLevels")
    ? this.getTotalLevels() - 1
    : LevelController.MAX_UNLOCKED_LEVEL_INDEX;
};

LevelController.prototype.getLevelIndexByName = function (levelName) {
  return this.levelTemplates.findIndex(
...
LevelController.prototype.selectNextLevel = function () {
  const nextLevelIndex = Math.min(
    this.getMaxUnlockedLevelIndex(),
    Math.min(LevelController.CURRENT_LEVEL_INDEX + 1, this.getTotalLevels() - 1)
  );
...
LevelController.prototype.unlockNextLevel = function () {
  ...
  this.setMaxUnlockedLevelIndex(
    Math.max(
      this.getMaxUnlockedLevelIndex(),
      Math.min(this.getTotalLevels() - 1, nextLevelIndex)
    )
  );
  this.app.fire(EventTypes.SAVE_APP);
};
...
LevelController.prototype._destroyCurrentLevel = async function () {
  ...
  while (this.entity.droppedBricksContainer.children.length > 0)
    if (this.entity.droppedBricksContainer.children[0])
      this.entity.droppedBricksContainer.children[0].destroy();
  while (this.entity.levelContainer.children.length > 0)
    this.entity.levelContainer.children[0].destroy();
  while (this.entity.stickmansContainer.children.length > 0)
    this.entity.stickmansContainer.children[0].destroy();
...
LevelController.prototype._instantiateLevel = async function () {
  ...
  if (!levelTemplate) {
    return Promise.reject(
      `LevelController._instantiateLevel: Can not find level template "${levelName}" in the library`
    );
  }
...
LevelController.prototype._generateLevelColors = async function () {
  this.app.colorsStorage.prepareRoundColors(
    this.entity.level.totalEnemies + 1,
    SkinsManager.getInstance().getPlayerColor()
  ); //we should count player color too
  return Promise.resolve();
};
...
Level.prototype._initializeEntityProperties = function () {
  ...
  this._finishPlatform = this.entity
    .findByName("Platforms")
    .findByName("Platform_Finish");
  this._stairs = this.entity
    .findByName("Bridges")
    .find((entity) => entity.script && entity.script.stairs);

  if (!this._finishPlatform)
    console.error(
      "Platform_Finish platform is not found on level " + this.entity.name
    );
  ...
  Object.defineProperty(this.entity, "totalEnemies", {
    get() {
      ...
      if (GameConfig.getAttribute("debug", "overrideEnemiesAmount")) {
        console.warn(
          "Enemies amount limited to " +
            GameConfig.getAttribute("debug", "limitEnemiesAmount") +
            " via GameCongig->debug"
        );
...
StickmanLifeStatus.prototype.initialize = function () {
  ...
  this.entity.on(
    EventTypes.Stickman.ENTERED_PLATFORM,
    this.onEnterPlatform,
    this
  );
  this.entity.on(
    EventTypes.Stickman.LEAVED_PLATFORM,
  ...
  this.entity.on(
    EventTypes.Stickman.ENTERED_FINISH_PLATFORM,
    this.onEnterFinishPlatform,
    this
  );
  this.entity.on(
    EventTypes.Stickman.COLLIDED_WITH_OTHER_STICKMAN,
    this.onCollidedWithOtherStickman,
    this
  );
  ...
  this.on("destroy", () => {
    ...
    this.entity.off(
      EventTypes.Stickman.ENTERED_STAIRS,
      this.onEnterStairs,
      this
    );
    this.entity.off(
      EventTypes.Stickman.LEAVED_STAIRS,
      this.onLeaveStairs,
      this
    );
    this.entity.off(
...
StickmanLifeStatus.prototype.update = function (dt) {
  ...
  if (GameConfig.getAttribute("debug", "showFloorName")) {
    this.entity.nicknameTextField.element.text = this.entity.floor
      ? this.entity.floor.name
      : "-";
  }

  if (GameConfig.getAttribute("debug", "showVelocitySlowdown")) {
    this.entity.nicknameTextField.element.text = `${Math.floor(
      (this.entity.runningVelocity /
        GameConfig.getAttribute("movement", "velocity")) *
        100
    )}%`;
  }
};
...
StickmanLifeStatus.prototype.onSessionStarted = function () {
  this.entity.invincibleCountdown = GameConfig.getAttribute(
    "gameplay",
    "invincibleAfterRespawnDuration"
  );
};
...
StickmanLifeStatus.prototype.onEnterPlatform = function (platform) {
  ...
  if (
    this.lastPlatform &&
    (this.lastPlatform.isPlatform || this.lastPlatform.isFinishPlatform)
  ) {
    this.lastPlatform.ingoingStairs.forEach((stairs) =>
      stairs.setPassStatus(this.entity.color, true)
    );
...
StickmanLifeStatus.prototype.onCollidedWithOtherStickman = function (
  otherStickman,
  contactResult
) {
  ...
  const force = contactResult.contacts[0].normal
    .clone()
    .normalize()
    .scale(-1)
    .scale(GameConfig.getAttribute("movement", "kickScaleXZ"));
  force.y = GameConfig.getAttribute("movement", "kickForceY");
  this.entity.rigidbody.applyImpulse(force, new pc.Vec3(0, 0, 0));

  this.entity.delayedCall(
    GameConfig.getAttribute("gameplay", "respawnDelay"),
    () => {
      if (GameSession.getInstance().sessionPaused) {
        this.app.once(EventTypes.GameSession.SESSION_RESUMED, () =>
          GameSession.getInstance()
            .waitUntilSessionResumed()
            .then(() => this.respawn())
        );
...
StickmanLifeStatus.prototype.verifyStickmanPosition = function () {
  ...
  if (
    this.entity.alive &&
    stickmanPosition.y < GameConfig.getAttribute("gameplay", "groundY")
  ) {
    ...
    if (this.entity.isPlayer) {
      ...
      this._respawnDelay = this.entity.delayedCall(
        GameConfig.getAttribute("AI", "aiRespawnCooldown"),
        () =>
          GameSession.getInstance()
            .waitUntilSessionResumed()
            .then(() => this.respawnOnStart())
      );
    }
  }
};
...
StickmanLifeStatus.prototype.onRespawnPlayer = function () {
  if (this.entity.isPlayer && !this.entity.alive) {
    ...
    if (this.entity._backpackCapacityBeforeDied > 0) {
      this.app.fire(
        EventTypes.AWARD_PLAYER_STICKMAN_WITH_BRICKS,
        this.entity._backpackCapacityBeforeDied
      );
...
StickmanLifeStatus.prototype.respawnOnStart = function () {
  const targetPosition = this.lastPlatform
    ? this.lastPlatform.getSpawnPosition(this.entity.color)
    : new pc.Vec3(0, 1, 0);
...
GameSession.getInstance = function () {
  if (!GameSession._instance)
    console.error("GameSession is not initialized yet");
  ...
  Object.defineProperty(this.entity, "sessionPaused", {
    ...
    set(value) {
      if (this._sessionPaused != value) {
        ...
        this._app.fire(
          this._sessionPaused
            ? EventTypes.GameSession.SESSION_PAUSED
            : EventTypes.GameSession.SESSION_RESUMED
        );
        this.resetGravity();
      }
    },
...
GameSession.prototype._setEntityMethods = function () {
  ...
  this.entity.resetGravity = function () {
    this._app.systems.rigidbody.gravity.copy(
      this._sessionPaused ? this._zeroGravity : this._initialWorldGravity
    );
  }.bind(this.entity);
};
...
GameSession.prototype.onSessionStart = function () {
  Apicontroller.trackLevelStart({
    level: LevelController.getInstance().getCurrentLevelIndex() + 1,
  });
  window.famobi_analytics
    .trackEvent("EVENT_LEVELSTART", {
...
SoundController.prototype.initialize = function () {
  ...
  const savedSFXVolume = LocalStorageController.getSavedValue(
    Constants.Storage.SFX_VOLUME
  );
  ...
  const savedMusicVolume = LocalStorageController.getSavedValue(
    Constants.Storage.MUSIC_VOLUME
  );
...
SoundController.prototype.playSfx = function (key, debounceDelay) {
  ...
  if (this.soundSeriesMap.has(key)) {
    ...
    if (currentTimestamp - lastSeriesTimestamp <= seriesConfig.timeout) {
      this.soundStorage.sound.slot(key).pitch = pc.math.clamp(
        seriesConfig.startPitch +
          (seriesIndex * (seriesConfig.endPitch - seriesConfig.startPitch)) /
            (seriesConfig.steps - 1),
        seriesConfig.startPitch,
        seriesConfig.endPitch
      );
...
SoundController.prototype.chokeMusic = function () {
  this.musicSlotKeys.forEach((key) => {
    ...
    if (slot)
      slot.volume = slot._initialVolume * SoundController.musicVolume * 0.2;
  });
};
...
SoundController.prototype.unchokeMusic = function () {
  this.musicSlotKeys.forEach((key) => {
    const slot = this.soundStorage.sound.slot(key);
    if (slot)
      slot.volume = slot._initialVolume * SoundController.musicVolume * 1.0;
  });
};
...
pc.Application.prototype.unpauseGame = function (forced) {
  ...
  if (isPageVisible && (!adIsShowing || forced)) {
    ...
    famobi.log(
      "resuming game is not allowed now because ads are displaying or page is not visible..."
    );
  }
};
...
var getForcedModeProperties = function () {
  const forcedModePproperties =
    typeof famobi !== "undefined" && famobi.getFeatureProperties("forced_mode");
...
LocalStorageController.getActualSaveData = function () {
  ...
  currentSaveData[Constants.Storage.CURRENT_LEVEL] =
    LevelController.getInstance().getCurrentLevelIndex();
  currentSaveData[Constants.Storage.MAX_UNLOCKED_LEVEL] =
    LevelController.getInstance().getMaxUnlockedLevelIndex();
  currentSaveData[Constants.Storage.COINS] =
    CoinsStorage.getInstance().getTotalCoins();
  currentSaveData[Constants.Storage.MUSIC_VOLUME] =
    SoundController.musicStateLoaded ? SoundController.musicVolume : 0.5;
  currentSaveData[Constants.Storage.SFX_VOLUME] = SoundController.sfxStateLoaded
    ? SoundController.sfxVolume
    : 0.5;
  currentSaveData[Constants.Storage.UNLOCKED_SKINS] =
    SkinsManager.getInstance().getUnlockedSkins();
  currentSaveData[Constants.Storage.SELECTED_CHARACTER_SKIN] =
    SkinsManager.getInstance().getSelectedCharacterSkin();
  currentSaveData[Constants.Storage.SELECTED_BRICK_SKIN] =
    SkinsManager.getInstance().getSelectedBrickSkin();
  currentSaveData[Constants.Storage.SELECTED_COLOR_SKIN] =
    SkinsManager.getInstance().getSelectedColorSkin();
  currentSaveData[Constants.Storage.LEVEL_TIMES] =
    LevelStatsManager.getInstance().getLevelTimesArray();
...
UiElementAnimator.prototype.initialize = function () {
  ...
  if (this.appearManually) {
    this.entity.on(
      EventTypes.UI_ELEMENT.APPEAR_MANUALLY,
      this.doAppearManually,
      this
    );
  } else {
...
UiElementAnimator.prototype.doAppearManually = function () {
  ...
  if (this.appearManually) {
    this.entity.children.forEach((child) =>
      child.fire(EventTypes.UI_ELEMENT.APPEAR_MANUALLY)
    );
  }
};
    ...
    tweenCompletePromises.push(
      new Promise((resolve, reject) => {
        ...
        this.entity
          ...
          .to(
            { opacity: tweenConfig.targetOpacity },
            tweenConfig.duration,
            pc[tweenConfig.easing]
          )
          .delay(
            tweenConfig.delay *
              (this.delayDependsOnChildIndex
                ? this.entity.parent.children.indexOf(this.entity) + 1
                : 1)
          )
          .on("complete", () => resolve())
          .on("start", () => {
            if (this.disabledUntilTweenStarts) this.entity.enabled = true;
          })
          .start();
      })
    );
  ...
  if (this.appearPositionTween.enabled) {
    tweenCompletePromises.push(
      new Promise((resolve, reject) => {
        ...
        this.entity.setLocalPosition(
          this.initialPosition.x + tweenConfig.startDeltaPosition.x,
          this.initialPosition.y + tweenConfig.startDeltaPosition.y,
          this.initialPosition.z + tweenConfig.startDeltaPosition.z
        );
        this.entity
          .tween(this.entity.getLocalPosition())
          .to(
            tweenConfig.toCustomPosition
              ? tweenConfig.customTargetPosition
    ...
    tweenCompletePromises.push(
      new Promise((resolve, reject) => {
        ...
        this.entity
          .tween(this.entity.getLocalPosition())
          .to(
            this.initialPosition.clone().add(tweenConfig.endDeltaPosition),
            tweenConfig.duration,
            pc[tweenConfig.easing]
          )
          .delay(
...
TestStickmanContainer.prototype.initialize = function () {
  ...
  if (this.disableMovement) {
    if (this.testStickmans) {
      this.testStickmans.forEach((stickman) =>
        stickman.script.destroy("stickmanMovement")
      );
    }
  }
  ...
  this.app.on(EventTypes.LEVEL.SPAWN_STICKMANS, () => {
    ...
    if (this.testStickmans) {
      this.testStickmans.forEach(
        (stickman) =>
          (stickman.color =
            SkinsManager.getInstance().getPlayerColor()) /* this.app.colorsStorage.getRandomColor()*/
      );
    }
  });
};
...
SkinsManager.getInstance = function () {
  if (!SkinsManager._instance)
    console.error("SkinsManager is not initialized yet");
...
SkinsManager.prototype.initialize = function () {
  ...
  this._unlockedSkinsKeys = new Set(
    LocalStorageController.getSavedValue(Constants.Storage.UNLOCKED_SKINS)
  );
  this._unlockedSkinsKeys.add("characterBasic");
  this._unlockedSkinsKeys.add("brickBasic");
  this._unlockedSkinsKeys.add("colorBlue");

  this._characterSkinKeys = new Set(
    this.characterSkins.map((skin) => skin.key)
  );
  ...
  this.selectedCharacterSkinKey =
    LocalStorageController.getSavedValue(
      Constants.Storage.SELECTED_CHARACTER_SKIN
    ) || "characterBasic";
  this.selectedBrickSkinKey =
    LocalStorageController.getSavedValue(
      Constants.Storage.SELECTED_BRICK_SKIN
    ) || "brickBasic";
  this.selectedColorSkinKey =
    LocalStorageController.getSavedValue(
      Constants.Storage.SELECTED_COLD_SKIN
    ) || "colorBlue";
...
SkinsManager.prototype.getSkinPrice = function (key) {
  if (this._characterSkinKeys.has(key))
    return this.characterSkins.find((elem) => elem.key === key).unlockPrice;
  if (this._brickSkinKeys.has(key))
    return this.brickSkins.find((elem) => elem.key === key).unlockPrice;
  if (this._colorSkinKeys.has(key))
    return this.colorSkins.find((elem) => elem.key === key).unlockPrice;
  return undefined;
};

SkinsManager.prototype.hasAvailableForPurchaseSkins = function () {
  const availableCoins = CoinsStorage.getInstance().getTotalCoins();
  return this.getLockedSkins().some(
    (skinKey) => this.getSkinPrice(skinKey) <= availableCoins
  );
};
...
SkinsManager.prototype.getLockedSkins = function () {
  return [
    ...Array.from(this._characterSkinKeys),
    ...Array.from(this._brickSkinKeys),
    ...Array.from(this._colorSkinKeys),
  ].filter((skinKey) => !this.isUnlocked(skinKey));
};
...
SkinsManager.prototype.getPlayerColor = function () {
  ...
  if (
    !this.currentColorSkin ||
    this.currentColorSkin.key !== this.selectedColorSkinKey
  ) {
    this.currentColorSkin = this.colorSkins.find(
      (skin) => skin.key === this.selectedColorSkinKey
    );
  }
...
SkinsManager.prototype.getPlayerSkinTemplate = function () {
  if (
    !this.currentCharacterSkin ||
    this.currentCharacterSkin.key !== this.selectedCharacterSkinKey
  ) {
    this.currentCharacterSkin = this.characterSkins.find(
      (skin) => skin.key === this.selectedCharacterSkinKey
    );
  }
...
SkinsManager.prototype.getRandomSkinKey = function () {
  return Utils.getRandomItem(
    Array.from(this._characterSkinKeys).filter(
      (skin) => skin != this.selectedCharacterSkinKey
    )
  );
};
...
SkinsManager.prototype.getBrickTemplate = function () {
  ...
  if (
    !this.currentBrickSkin ||
    this.currentBrickSkin.key !== this.selectedBrickSkinKey
  ) {
    this.currentBrickSkin = this.brickSkins.find(
      (skin) => skin.key === this.selectedBrickSkinKey
    );
  }
...
SkinPanel.prototype._initializeEntityProperties = function () {
  ...
  [
    ...
    "BuyButtonGroup",
    "SkinPricePad",
    "SkinPricePadUnavailable",
    "SkinPriceText",
  ].forEach(
    (itemKey) => (this.entity[itemKey] = this.entity.findByName(itemKey))
  );
...
SkinPanel.prototype.updateView = function () {
  const unlockedStatus = SkinsManager.getInstance().isUnlocked(
    this.entity.skinKey
  );
  const isCurrentlySelected = SkinsManager.getInstance().isSelected(
    this.entity.skinKey
  );

  if (unlockedStatus) {
    this.entity.IconPad.element.color = new pc.Color(
      113 / 255,
      1,
      119 / 255,
      1
    ...
    this.entity.SkinPricePad.enabled =
      CoinsStorage.getInstance().hasEnoughCoins(this.entity._unlockPrice);
    this.entity.SkinPricePadUnavailable.enabled =
      !this.entity.SkinPricePad.enabled;
    this.entity.IconPad.element.color =
      CoinsStorage.getInstance().hasEnoughCoins(this.entity._unlockPrice)
        ? new pc.Color(1, 1, 0, 1)
        : new pc.Color(0.8, 0.8, 0.8, 0.5);
  }
};
  ...
  if (!SkinsManager.getInstance().isUnlocked(this.entity.skinKey)) {
    if (CoinsStorage.getInstance().hasEnoughCoins(this.entity._unlockPrice)) {
      ...
      this.entity.delayedCall(
        500,
        () =>
          (this.entity.SkinPriceText.element.color = new pc.Color(1, 1, 1, 1))
      );
...
AccessoryModelFinder.prototype.initialize = function () {
  this.entity.overrideTargetModel =
    this.entity.parent.parent.findByName("StickmanModel");
};
...
StickmanAccessory.prototype._assignEntityMethods = function () {
  ...
  this.entity.setAccessory = function (accessoryKey) {
    ...
    const accessoryTemplate =
      SkinsManager.getInstance().getSkinTemplate(accessoryKey);
...
VolumeSlider.prototype.initialize = function () {
  ...
  this.entity.on(
    "setSliderValue",
    (percentage) => this.updateView(percentage, true),
    this
  );
};
...
VolumeSlider.prototype.onTap = function (event) {
  if (event.changedTouches && event.changedTouches[0]) {
    this._sliderHolding = true;
    this.inputDownPosition = new pc.Vec2(
...
VolumeSlider.prototype.onMouseMove = function (x, y) {
  if (this._sliderHolding && this.inputDownPosition) {
    ...
    sliderPosition.x = pc.math.clamp(
      sliderPosition.x + dx,
      0,
      this.maxMaskWidth
    );
...
VolumeSlider.prototype.onTouchMove = function (x, y) {
  if (this._sliderHolding && this.inputDownPosition) {
    const sliderPosition = this.slider.getLocalPosition();
    const dx =
      (x - this.inputDownPosition.x) / this.uiContainer.screen.scaleBlend;
    sliderPosition.x = pc.math.clamp(
      sliderPosition.x + dx,
      0,
      this.maxMaskWidth
...
VolumeSlider.prototype.updateView = function (percentage, skipEventFiring) {
  const sliderX = pc.math.clamp(
    percentage * this.maxMaskWidth,
    0,
    this.maxMaskWidth
  );
...
CoinsStorage.prototype.initialize = function () {
  ...
  /* initialize varibles */
  this._numCoins = CoinsStorage.coinsSetViaForcedMode
    ? CoinsStorage.forcedModeCoins
    : LocalStorageController.getSavedValue(Constants.Storage.COINS);
...
LevelStatsManager.getInstance = function () {
  if (!LevelStatsManager._instance)
    console.error("LevelStatsManager is not initialized yet");
...
LevelStatsManager.prototype.initialize = function () {
  ...
  this.levelTimes =
    LocalStorageController.getSavedValue(Constants.Storage.LEVEL_TIMES) || [];
  ...
  this.app.on(
    EventTypes.Stats.PLAYER_PICKED_UP_A_BRICK,
    this._onPlayerPickedUpABrick,
    this
  );
};
...
LevelStatsManager.prototype.getLevelTimeReward = function () {
  return Math.floor(
    pc.math.clamp(
      (GameConfig.getAttribute("gameplay", "maxExtraSeconds") -
        this._levelTime) *
        GameConfig.getAttribute("gameplay", "rewardPerExtraSecond"),
      0,
      10000
    )
  );
...
LevelStatsManager.prototype.getExtraBricksReward = function () {
  return (
    this._bricksLeft * GameConfig.getAttribute("gameplay", "rewardPerBrick")
  );
};
...
LevelStatsManager.prototype.onLevelPassed = function () {
  const prevBestTime =
    this.levelTimes[LevelController.getInstance().getCurrentLevelIndex()];
...
LevelButton.prototype._assignEntityMethods = function () {
  ...
  this.entity.updateView = function () {
    ...
    /* set level time */
    const levelBestTime = LevelStatsManager.getInstance().getLevelBestTime(
      this._levelIndex
    );
    this.LevelBestTimeText.element.text = levelBestTime
      ? `${levelBestTime.toFixed(2)}s`
      : `---`;

    /* load level preview picture */
    const levelName = LevelController.getInstance().getLevelNameByIndex(
      this._levelIndex
    );
    ...
    if (
      ...
      this.LevelPreviewPicFrame.element.color = new pc.Color().fromString(
        "#FFFFFF"
      );
...
Jumper.prototype._initializeEntityProperties = function () {
  if (!this.targetEntity)
    return console.error(
      "Jumper target enttiy is not set on " + this.entity.path
    );
...
PlatformColorChager.prototype.changePlatformColor = function () {
  const platformMaterial = GameConfig.getAttribute(
    "platform",
    "material"
  ).resource;

  const randomValue = pc.math.random(
    this.mainColorValueLimits.x,
    this.mainColorValueLimits.y
  );
  const randomColor = Utils.shuffle([
    randomValue,
    1 - randomValue,
    pc.math.random(0, Math.min(randomValue, 1 - randomValue)) / 2,
  ]).map(
    (value, index) =>
      value * this.colorDampingFactor * this.rgbDamping[["x", "y", "z"][index]]
  );
  if (this.useEmissive) {
...
AvailableSkinsNotification.prototype.onCoinsAmountChanged = function () {
  this.entity.enabled =
    SkinsManager.getInstance().hasAvailableForPurchaseSkins();
};
...
Elevator.prototype.postUpdate = function (dt) {
  if (
    GameSession.getInstance().sessionActive &&
    this._stickmansInside.size > 0
  ) {
    const yPosition =
      this.entity.ElevatorFloor.getPosition().y +
      this.entity.ElevatorFloor.collision.height / 2;
    this._stickmansInside.forEach((stickman) => {
      const stickmanPosition = stickman.getPosition();
      stickman.rigidbody.teleport(
        new pc.Vec3(
          stickmanPosition.x,
          Math.max(stickmanPosition.y, yPosition + 0.8 + 0.001),
          stickmanPosition.z
        )
      );
    });
...
Elevator.prototype._updateBottomDoors = function () {
  this.entity.DoorBottomLeft.setLocalEulerAngles(
    0,
    -35 * this.bottomDoorProgress,
    0
  );
  this.entity.DoorBottomRight.setLocalEulerAngles(
    0,
    35 * this.bottomDoorProgress,
    0
  );
...
Elevator.prototype._updateElevatorMovement = function () {
  ...
  const targetY =
    this.floorDownY +
    this.elevatorProgress * (this.floorTopY - this.floorDownY);
  this.entity.ElevatorFloor.rigidbody.teleport(
    position.x,
    position.y + targetY,
    position.z
  );
...
Elevator.prototype._initializeEntityProperties = function () {
  if (!this.entity.ExitNode)
    return console.error(
      "ExitNode is not found on elevator " + this.entity.path
    );
  if (!this.platformTo)
    return console.error(
      "Platform-To is not set on elevator " + this.entity.path
    );
...
ScrollingTexture.prototype.update = function (dt) {
  ...
  this.entity.model.material.diffuseMapOffset.set(
    this.offset.x % 1,
    this.offset.y % 1
  );
...
ScreenBlocker.prototype.showTransitionScreen = function (
  duration = 0.3,
  ...
  this.entity.element.color = new pc.Color().fromString(
    splashColor || this.initialColor.toString()
  );
...
ScreenBlocker.prototype.hideTransitionScreen = function (
  duration = 0.25,
  onCompleteCallback = null
) {
  this.resetActiveTween();
...
BricksCache.getInstance = function () {
  if (!BricksCache._instance)
    throw new Error("BricksCache is not initialized yet");
...
BricksCache.prototype.initialize = function () {
  ...
  if (!this.cacheEntityEnabled)
    return console.error("Cache-enabled entity s not set for BricksCache");
  if (!this.cacheEntityDisabled)
    return console.error("Cache-disabled entity is not set for BricksCache");
...
BricksCache.prototype._getFromCache = function (preferredColor) {
  if (this.cache.length > 0) {
    ...
    const brick = SkinsManager.getInstance()
      .getBrickTemplate()
      .resource.instantiate();
    return brick;
  }
...
BricksCache.prototype.disposeBrick = function (entity) {
  ...
  if (entity.name !== this.cachedBrickTemplateName) {
    console.warn(
      "Disposed brick belongs to a different type: " +
        entity.name +
        " instead of " +
        this.cachedBrickTemplateName
    );
...
Apicontroller.initTracking = function () {
  ...
  window.famobi_tracking.init(
    Apicontroller.FAMOBI_TRACKING_KEY,
    null,
    100,
    true,
    true
  ...
  famobi.log(
    "Tracking API initialized with key " + Apicontroller.FAMOBI_TRACKING_KEY
  );
};
...
AIManager.prototype.getStrongestBotsOfPlatform = function (platform) {
  return this.entity.stickmansAI
    .slice()
    .sort((a, b) => b.backpack.capacity - a.backpack.capacity);
};

AIManager.prototype.isPlayerAhead = function (enemy) {
  return (
    this.entity.playerStickman.alive &&
    this.entity.playerStickman.lastPlatformID > enemy.lastPlatformID
  );
};
