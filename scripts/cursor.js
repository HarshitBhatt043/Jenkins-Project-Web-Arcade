...
const userstrokeCtx = userstrokeCanvas.getContext("2d", { alpha: !1 }),
  CURSORS = [];
class Cursor {
  constructor(t) {
    (this.x = 0),
  ...
  canvasCursorEnter(t, s) {
    ...
    const e = t(this),
      i = s(this);
    Cursor.interpolateCursorBorderPosition(e, i),
      (this.prevX = i[0]),
      (this.prevY = i[1]),
      (this.x = e[0]),
      (this.y = e[1]);
  ...
  canvasCursorLeave(t) {
    ...
    const s = t(this);
    Cursor.interpolateCursorBorderPosition([this.prevX, this.prevY], s),
      (this.x = s[0]),
      (this.y = s[1]);
  }
  ...
  static interpolateCursorBorderPosition(t, s) {
    var e = t[1] - s[1],
      i = t[0] - s[0];
    0 === e && (e = 0.001), 0 === i && (i = 0.001);
    const n = e / i,
      o = t[1] - n * t[0];
    return (
      s[0] < 0
        ? ((s[0] = 0), (s[1] = o))
        : s[0] > MAX_X_IDX && ((s[0] = MAX_X_IDX), (s[1] = n * MAX_X_IDX + o)),
      s[1] < 0
        ? ((s[1] = 0), (s[0] = (0 - o) / n))
        : s[1] > MAX_Y_IDX &&
          ((s[1] = MAX_Y_IDX), (s[0] = (MAX_Y_IDX - o) / n)),
      (s[0] = Math.floor(s[0])),
      (s[1] = Math.floor(s[1])),
      (s[0] = Math.max(Math.min(s[0], MAX_X_IDX), 0)),
      (s[1] = Math.max(Math.min(s[1], MAX_Y_IDX), 0)),
      s
    );
  }
  drawStroke() {
    if (!this.isDown) return;
    if (!this.inCanvas && this.prevX === this.x && this.prevY === this.y)
      return;
    ...
    userstrokeCanvas.width < d && (userstrokeCanvas.width = d),
      userstrokeCanvas.height < M && (userstrokeCanvas.height = M),
      userstrokeCtx.beginPath(),
      userstrokeCtx.rect(0, 0, d, M),
      (userstrokeCtx.fillStyle = "rgba(0, 0, 0, 1)"),
      userstrokeCtx.fill(),
      c === f && v === C
        ? (userstrokeCtx.beginPath(),
          (userstrokeCtx.lineWidth = 0),
          (userstrokeCtx.fillStyle = e),
          userstrokeCtx.arc(c, v, PENSIZE / 2, 0, TWO_PI),
          userstrokeCtx.fill())
        : ((userstrokeCtx.lineWidth = PENSIZE),
          (userstrokeCtx.strokeStyle = e),
          (userstrokeCtx.lineCap = "round"),
          userstrokeCtx.beginPath(),
          userstrokeCtx.moveTo(c, v),
          userstrokeCtx.lineTo(f, C),
    ...
    if (E > S || X > I) console.log("Bug in userstroke drawing");
      ...
      for (l = X; l !== I; l++) {
        ...
        for (m = E; m !== S; m++) {
          ...
          if (4278190080 !== p[m + i]) {
            const i = n + e;
            (s || gameImagedata32[i] === BACKGROUND) &&
              (gameImagedata32[i] = t);
          }
        }
      }
...
class Mouse extends Cursor {
  ...
  canvasMouseDown(t) {
    ...
    this.shiftPressed && !t.shiftKey && (this.shiftPressed = !1),
      this.shiftPressed &&
        ((this.shiftStartX = s[0]),
        (this.shiftStartY = s[1]),
        (this.lineDirection = Mouse.NO_DIRECTION)),
      super.canvasCursorDown(s[0], s[1]);
  }
  canvasMouseMove(t) {
    const s = this.canvas;
  ...
  canvasMouseEnter(t) {
    ...
    super.canvasCursorEnter(
      ...
      this.isDown &&
        this.shiftPressed &&
        this.lineDirection === Mouse.NO_DIRECTION &&
        ((this.shiftStartX = this.prevX), (this.shiftStartY = this.prevY));
  }
  canvasMouseLeave(t) {
    const s = this.canvas;
  ...
  documentMouseDown(t) {
    ...
    const s = this.canvas;
    this.shiftPressed && !t.shiftKey && (this.shiftPressed = !1),
      this.shiftPressed && (this.lineDirection = Mouse.NO_DIRECTION),
      super.documentCursorDown(t, function () {
        return Mouse.getMousePos(t, !1, s);
      });
  }
  static getMousePos(t, s, e) {
    ...
    return (
      s
        ? ((i = t.offsetX),
          (n = t.offsetY),
          i < 0 ? (i = 0) : i >= width && (i = MAX_X_IDX),
          n < 0 ? (n = 0) : n >= height && (n = MAX_Y_IDX))
        : ((i = t.pageX - docOffsetLeft(e)), (n = t.pageY - docOffsetTop(e))),
      [Math.round(i), Math.round(n)]
    );
  }
  documentKeyDown(t) {
    t.shiftKey &&
      (this.shiftPressed ||
        ...
        this.isDown &&
          this.inCanvas &&
          ((this.shiftStartX = this.x), (this.shiftStartY = this.y))));
  }
  documentKeyUp(t) {
  ...
  documentVisibilityChange(t) {
    "hidden" == document.visibilityState &&
      (this.documentMouseUp(null), (this.shiftPressed = !1)),
      super.documentVisibilityChange(t);
  }
  handleShift() {
    ...
    if (this.lineDirection === Mouse.NO_DIRECTION) {
      ...
      Math.abs(e - i) < 5
        ? (this.lineDirection =
            s * t < 0 ? Mouse.DIAGONAL_DOWN : Mouse.DIAGONAL_UP)
        : (this.lineDirection = e > i ? Mouse.HORIZONTAL : Mouse.VERTICAL);
    }
    const t = this.lineDirection;
    if (t === Mouse.HORIZONTAL)
      (this.prevY = this.shiftStartY), (this.y = this.shiftStartY);
    else if (t === Mouse.VERTICAL)
      (this.prevX = this.shiftStartX), (this.x = this.shiftStartX);
      ...
      const s = t === Mouse.DIAGONAL_DOWN ? -1 : 1,
        ...
        i = s * this.x + e,
        n = (this.y - e) / s;
      distance(n, this.y, this.shiftStartX, this.shiftStartY) >
      distance(this.x, i, this.shiftStartX, this.shiftStartY)
        ? (this.x = n)
        : (this.y = i);
    }
    return !1;
