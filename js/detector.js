var detector = {
  ...
  animate: function (time) {
    var duration =
      typeof time !== "undefined" ? time - detector.lastRender : 16;
  ...
  init: function (baseSize) {
    ...
    var backingStoreRatio =
      detector.core.ctx.webkitBackingStorePixelRatio ||
      ...
      detector.core.ctx.backingStorePixelRatio ||
      1;
  ...
  coreDraw: function () {
    ...
    for (var k = 3; k >= 1; k--) {
      ...
      ctx.moveTo(
        cx +
          (detector.radius.mucal +
            k * detector.radius.mucalLight +
            k * detector.radius.mucalDark) *
            Math.cos(Math.PI * muSplit) *
            detector.ratio,
        cy +
          (detector.radius.mucal +
            k * detector.radius.mucalLight +
            k * detector.radius.mucalDark) *
            Math.sin(Math.PI * muSplit) *
            detector.ratio
      );
      for (var i = 1; i <= 13; i++) {
        ctx.lineTo(
          cx +
            (detector.radius.mucal +
              k * detector.radius.mucalLight +
              k * detector.radius.mucalDark) *
              Math.cos(Math.PI * i * muSplit) *
              detector.ratio,
          cy +
            (detector.radius.mucal +
              k * detector.radius.mucalLight +
              k * detector.radius.mucalDark) *
              Math.sin(Math.PI * i * muSplit) *
              detector.ratio
        );
      }
      ctx.stroke();
    ...
    ctx.beginPath();
    ctx.moveTo(
      cx + detector.radius.mucal * Math.cos(Math.PI * muSplit) * detector.ratio,
      cy + detector.radius.mucal * Math.sin(Math.PI * muSplit) * detector.ratio
    );
    for (var i = 1; i <= 13; i++) {
      ctx.lineTo(
        cx +
    ...
    ctx.arc(
      cx,
      cy,
      detector.radius.darkRing2 * detector.ratio,
      0,
      Math.PI * 2,
      true
    );
    ...
    ctx.arc(
      cx,
      cy,
      detector.radius.lightRingSpace * detector.ratio,
      0,
      Math.PI * 2,
      true
    );
    ...
    ctx.arc(
      cx,
      cy,
      detector.radius.lightRing * detector.ratio,
      0,
      Math.PI * 2,
      true
    );
    ...
    ctx.arc(
      cx,
      cy,
      detector.radius.darkRing1Space * detector.ratio,
      0,
      Math.PI * 2,
      true
    );
    ...
    ctx.arc(
      cx,
      cy,
      detector.radius.darkRing1 * detector.ratio,
      0,
      Math.PI * 2,
      true
    );
    ...
    ctx.arc(
      cx,
      cy,
      detector.radius.ecal * detector.ratio,
      0,
      Math.PI * 2,
      true
    );
    ...
    for (var i = 0; i < 20; i++) {
      ...
      ctx.moveTo(
        cx +
          detector.radius.ecal *
            Math.cos((Math.PI * i) / calSplit) *
            detector.ratio,
        cy +
          detector.radius.ecal *
            Math.sin((Math.PI * i) / calSplit) *
            detector.ratio
      );
      ctx.lineTo(
        cx +
          detector.radius.hcal *
            Math.cos((Math.PI * i) / calSplit) *
            detector.ratio,
        cy +
          detector.radius.hcal *
      ...
      ctx.arc(
        cx,
        cy,
        detector.radius.hcal * detector.ratio,
        (Math.PI * i) / calSplit,
        (Math.PI * (i + 1)) / calSplit,
        false
      );
      ctx.lineTo(
        cx +
          detector.radius.ecal *
            Math.cos((Math.PI * (i + 1)) / calSplit) *
            detector.ratio,
        cy +
          detector.radius.ecal *
            Math.sin((Math.PI * (i + 1)) / calSplit) *
            detector.ratio
      );
      ctx.lineTo(
        cx +
    ...
    ctx.arc(
      cx,
      cy,
      detector.radius.silicon * detector.ratio,
      0,
      Math.PI * 2,
      true
    );
    ...
    ctx.arc(
      cx,
      cy,
      detector.radius.siliconInner * detector.ratio,
      0,
      Math.PI * 2,
      true
    );
