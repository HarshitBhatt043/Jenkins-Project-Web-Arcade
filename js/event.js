function ParticleEvent(type, count, external) {
  ...
  switch (this.type.name) {
    case "electron":
      this.length =
        detector.radius.siliconSpace * detector.ratio +
        Math.round(
    ...
    case "jet":
      this.length =
        detector.radius.ecal * detector.ratio +
        Math.round(
          (detector.radius.mucal * detector.ratio -
            detector.radius.ecal * detector.ratio) *
            Math.random()
        );
      this.direction = Math.random() * Math.PI * 2;
    ...
    case "muon":
      this.length =
        detector.radius.mucal * detector.ratio +
        3 * detector.radius.mucalDark * detector.ratio +
        Math.round(
          (4 * detector.radius.mucalLight * detector.ratio +
            2 * detector.radius.mucalDark * detector.ratio) *
            Math.random()
        );
      this.direction = Math.random() * Math.PI * 2;
    ...
    cy +
      this.sign *
        Math.round(
          Math.sqrt(
            Math.abs(
              this.radius * this.radius - (this.length * this.length) / 4
            )
          )
        ),
    this.radius,
    (-this.sign * Math.PI) / 2 - Math.asin(this.length / (2 * this.radius)),
    (-this.sign * Math.PI) / 2 + Math.asin(this.length / (2 * this.radius)),
    false
  );
  ctx.stroke();
