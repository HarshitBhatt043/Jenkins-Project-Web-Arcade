var GameObjects = (function () {
  ...
  GameObject.prototype.loadState = function (state) {
  ...
  Research.prototype.research = function (lab) {
    if (lab && lab.research(this.state.cost, this.state.reputation)) {
      this.state.level++;
      if (
        this.state.info_levels.length > 0 &&
  ...
  Achievement.prototype.validate = function (lab, allObjects, saveTime) {
    ...
    if (
      allObjects.hasOwnProperty(this.targetKey) &&
      ...
      this.state.timeAchieved =
        lab.state.time + new Date().getTime() - saveTime;
