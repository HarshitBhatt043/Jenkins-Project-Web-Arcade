...
var UI = (function () {
    ...
    var resize = function () {
      ...
      if ($(window).width() < 600) {
        var newWidth = Math.max(
          $(window).width() - ($(window).height() - 90 + 10),
          300
        );
      ...
      if ($(window).width() >= 1200) {
        ...
        var newWidth =
          $(window).width() -
          Math.max($(window).width() - ($(window).height() - 90 + 10), 300) -
          10;
  ...
  var showUpdateValue = function (ident, num) {
    if (num != 0) {
      ...
      if (num > 0) {
        ...
        insert = $("<div></div>").attr("class", "update-minus").html(formatted);
      }
  ...
  var showAchievement = function (obj) {
    ...
    alert +=
      '<span class="fa ' +
      obj.icon +
      ' alert-glyph"></span> <span class="alert-text">' +
      obj.description +
      "</span>";
    alert += "</div>";
  ...
  if (typeof $.cookie("cookielaw") === "undefined") {
    ...
    alert +=
      '<i class="fa fa-info-circle alert-glyph"></i> <span class="alert-text">Particle Clicker uses local storage to store your current progress.</span>';
    alert += "</div>";
    alert = $(alert);
    alert.find("button").click(function () {
      $.cookie("cookielaw", "informed", { expires: 365 });
      $("#cookielaw").slideUp(300, function () {
        $("#cookielaw").remove();
      });
    });
  ...
  if (typeof $.cookie("cern60") === "undefined") {
    ...
    alert +=
      '<i class="fa fa-area-chart alert-glyph"></i> <span class="alert-text"><a class="alert-link" href="http://home.web.cern.ch/about/updates/2014/12/take-part-cern-60-public-computing-challenge" target="_blank">Join the CERN 60 computing challenge!</a></span>';
    alert += "</div>";
    alert = $(alert);
    alert.find("button").click(function () {
      $.cookie("cern60", "closed", { expires: 365 });
      $("#cern60").slideUp(300, function () {
        $("#cern60").remove();
      });
    });
...
(function () {
  ...
  function onchange(evt) {
    ...
    if (evt.type in evtMap) detector.visible = evtMap[evt.type] == "visible";
    else detector.visible = !this[hidden];
  }
})();
