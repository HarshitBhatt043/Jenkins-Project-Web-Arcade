...
var Game = new (function () {
  ...
  function init() {
    ...
    $("[data-size]").each(function (i, el) {
      ...
      $el.on("touchstart mousedown", function (evt) {
        ...
        var size =
          sizes[$(evt.target).closest("[data-size]").attr("data-size") * 1 - 1];
        loadGame(size);
      });
    });
    resize();
  ...
  function resize() {
    ...
    var scoreSize = Math.round(containerSize * 0.1);
    $("#score").css({
    ...
    $("#digits")
      .width($("#titlegrid table").width())
      .height($("#titlegrid table").height());
    $("#digits").css({
      "line-height": Math.round($("#titlegrid table").height() * 0.92) + "px",
      "font-size": $("#titlegrid table").height() * 0.5 + "px",
    });

    var topVSpace = Math.floor(
  ...
  function endGame() {
    ...
    // shift
    if (!currentPuzzle.isTutorial) Levels.finishedSize(grid.width);
  }
  ...
  function doAction(action, value) {
    switch (action) {
      case "close-titleScreen":
        if (!tutorialPlayed()) startTutorial();
        else showMenu();
        break;
      case "show-menu":
        clearTimeout(checkTOH);
        Tutorial.end();
        if (grid) grid.hint.clear();
        showMenu();
        break;
      case "back":
        if (gameEnded) return doAction("show-menu");
        clearTimeout(checkTOH);
        Tutorial.end();
        quitCurrentGame();
        break;
      ...
      case "help":
        ...
        if (grid.hint.visible) grid.hint.clear();
        else {
          grid.hint.clear();
          grid.hint.next();
        }
  ...
  function checkForLevelComplete() {
    if (grid.emptyTileCount > 0) return;
  ...
  function startTutorial() {
    ...
    if (!tutorialPlayed()) startedTutorial = false;
  ...
  function backButtonPressed() {
    if (onHomeScreen) navigator.app.exitApp();
    else doAction("back");
  }

  function getOjoo() {
    if (!remainingOjoos.length) remainingOjoos = Utils.shuffle(ojoos.slice(0));
  ...
  function undo() {
    if (!undoStack.length) {
      ...
      if (!undone) grid.hint.show("That's the undo button.");
      else grid.hint.show("Nothing to undo.");
      return;
    }
  ...
  function updateTweetUrl(size) {
    var msg =
        "#0hh1 I just completed a " +
        size +
        " x " +
        size +
        " puzzle and my score is " +
        getScore() +
        ".",
