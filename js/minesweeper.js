...
$(function () {
  ...
  var action = {
    ...
    click: function (e) {
      ...
      var x = Math.floor(
          (e.pageX - globals.canvas[0].offsetLeft - 1) / defaults.celSize
        ),
        y = Math.floor(
          (e.pageY - globals.canvas[0].offsetTop - 1) / defaults.celSize
        ),
      ...
      if (
        ...
        if (globals.firstClick === true) {
          ...
          // Set number of mines
          containers.mines.html(
            "You have to find " + globals.totalMines + " mines to win."
          );
    ...
    hover: function (e) {
      if (!globals.gameover) {
        ...
        if (
          typeof pX !== "undefined" &&
          globals.revealedMap[pX][pY] !== 1 &&
          globals.flagMap[pX][pY] !== 1
        ) {
          globals.context.fillStyle = defaults.celColor;
            ...
            var alpha = 0.1,
              squareFade = setInterval(function () {
                ...
                if (globals.mineMap[x][y] !== -1) {
                  ...
                  globals.context.fillText(
                    globals.mineMap[x][y],
                    x * defaults.celSize + 5,
                    y * defaults.celSize + 16
                  );
                }
            ...
            for (var i = -1; i <= 1; i++) {
              for (var j = -1; j <= 1; j++) {
                ...
                if (
                  ...
                  x + i >= 0 &&
                  y + j >= 0 &&
                  x + i <= globals.squaresX &&
                  y + j <= globals.squaresX
                ) {
    ...
    flag: function (flag, x, y) {
      ...
      if (globals.flagMap[x][y] !== 1) {
        ...
        // Remove flag image
        var img = globals.context.createImageData(
          defaults.celSize,
          defaults.celSize
        );
      ...
      containers.mines.html(
        "You have to find " +
          (globals.totalMines - globals.totalFlags) +
          " mines to win."
      );
    ...
    generateMines: function () {
      ...
      for (var i = 0; i < globals.squaresX; i++) {
        ...
        for (var j = 0; j < globals.squaresY; j++) {
          globals.mineMap[i][j] = Math.floor(
            Math.random() * defaults.difficulty - 1
          );
  ...
  var scores = {
    display: function () {
      if (typeof Storage !== "undefined") {
        ...
        if (typeof localStorage.scores !== "undefined") {
          ...
          containers.scoreboard.html(
            "<tr><th>Name</th><th>Mines</th><th>Seconds</th></tr>"
          );

          $.each(lScores, function () {
            containers.scoreboard.append(
              "<tr><td>" +
                this[0] +
                "</td><td>" +
                this[2] +
          ...
          containers.scoreboard.html(
            "<tr><td>You have not won any games yet :(</td></tr>"
          );
        }
      } else {
        containers.scoreboard.html(
          "<tr><td>Unfortunately, your browser does not support local storage</td></tr>"
        );
      }
    },
    ...
    save: function () {
      if (typeof Storage !== "undefined") {
        var name = prompt(
            "Your score is being stored. Please enter your name",
            "Name"
          ),
          score = [
            name,
      ...
      globals.currentAnimation = setInterval(function () {
        ...
        for (var i = 0; i <= arrow.length; i++) {
          if (arrow[i] !== undefined) {
            util.roundRect(
              arrow[i][0] * defaults.celSize,
              arrow[i][1] * defaults.celSize,
              defaults.celSize - 1,
              defaults.celSize - 1
            );
          }
  ...
  var util = {
    ...
    is: function (what, x, y) {
      ...
      if (
        typeof p[what][x] !== "undefined" &&
        typeof p[what][x][y] !== "undefined" &&
        p[what][x][y] > -1
      ) {
        return true;
      } else {
