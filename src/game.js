MG.game = (function () {
  ...
  var playCrashAnimation = function () {
    // Get the path element with the ID "explosion"
    var explosionPath = document.getElementById("explosion");

    // Check if the path element exists
    if (explosionPath) {
      // Get the first animation element within the path (assuming it's the correct animation)
      var animation = explosionPath.querySelector("animate");

      // Check if an animation element was found
      if (animation) {
        // Play the animation
        animation.beginElement();
        explosionPath.setAttribute("visibility", "visible");

        // Set a timeout to hide the element after the animation duration (400 milliseconds)
        setTimeout(function () {
  ...
  return {
    ...
    update: function (dt) {
      ...
      if (!MG.barrierQueue.isEmpty()) {
        if (
          MG.missile.getOffset() < MG.MISSILE_LENGTH &&
          ...
          if (
            barrier.collides(
              MG.missile.getPosition().x,
              MG.missile.getPosition().y
            )
          ) {
            // CRASH
            MG.missile.onCrash();
      ...
      while (
        MG.barrierQueue.numBarriers() <
        MG.LINE_OF_SIGHT / MG.BARRIER_SPACING
      ) {
      ...
      switch (mState) {
        case GameState.RUNNING:
          mProgress =
            1 -
            (mBarriersToPass * MG.BARRIER_SPACING + MG.missile.getOffset()) /
              (LEVEL_NUM_BARRIERS * MG.BARRIER_SPACING);
