  ...
  return {
    ...
    update: function (dt) {
      switch (mState) {
        case MissileState.AUTOPILOT:
          ...
          if (mDriftCounter < 0) {
            ...
            mDriftVelX =
              (MG.TUNNEL_RADIUS * (Math.random() - 0.5) - mTargetX) / 1.5;
            mDriftVelY =
              (MG.TUNNEL_RADIUS * (Math.random() - 0.5) - mTargetY) / 1.5;
          }
      ...
      if (mState === MissileState.CRASHED) {
        ...
        mVelocity +=
          (dt * MG.BARRIER_SPACING * mVelocity) /
          (mOffset - MG.BARRIER_SPACING);
      } else {
        mVelocity +=
          (dt * (mTargetVelocity - mVelocity)) / ACCELERATION_TIME_CONSTANT;
      }
