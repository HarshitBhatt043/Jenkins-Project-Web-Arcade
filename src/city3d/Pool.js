...
export class Pool {
  constructor(
    ...
    if (this.isWithNormal)
      this.imgSrc.push("tiles_n.png", "building_n.png", "town_n.png");
    if (this.isWithRoughness)
      this.imgSrc.push("tiles_r.png", "building_r.png", "town_r.png");
  ...
  defineCanvas() {
    ...
    if (this.isWithNormal)
      this.canvas["tiles_n"] = this.makeCanvas("tiles_n", true);
    if (this.isWithRoughness) {
  ...
  defineTextures() {
    ...
    if (this.isWithNormal) {
      ...
      this.filterTexture(this.textures["town_n"], {
        flip: false,
        normal: true,
      });

      this.textures["building_n"] = new THREE.Texture(this.imgs["building_n"]);
      this.filterTexture(this.textures["building_n"], {
        flip: false,
        normal: true,
      });
    }
    ...
    if (this.isWithRoughness) {
      ...
      this.filterTexture(this.textures["town_r"], {
        flip: false,
        normal: true,
      });

      this.textures["building_r"] = new THREE.Texture(this.canvas.building_r);
      this.filterTexture(this.textures["building_r"], {
        flip: false,
        normal: true,
      });
    }
  ...
  makePixelData(name) {
    ...
    for (let i = 0; i < 240; i++) {
      ...
      if (name === "tiles_n")
        this.tiles.normal[i] = new THREE.DataTexture(data, pix, pix);
      else if (name === "tiles_r")
        this.tiles.roughness[i] = new THREE.DataTexture(data, pix, pix);
    ...
    switch (name) {
      ...
      case "world":
        g = {
          ...
          sprite: [o.train, o.elico.clone(), o.plane.clone()],
