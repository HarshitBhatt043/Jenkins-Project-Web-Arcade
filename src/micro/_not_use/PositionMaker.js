...
function PositionMaker(width, height) {
  if (arguments.length < 2)
    throw new Error(
      "PositionMaker called with too few arguments " +
        [].toString.apply(arguments)
    );

  if (typeof width !== "number" || typeof height !== "number")
    throw new Error(
      "PositionMaker called with invalid width or height " +
        [].toString.apply(arguments)
    );

  if (width < 0 || height < 0)
    throw new Error(
      "PositionMaker called with out-of-bounds width or height " +
        [].toString.apply(arguments)
    );
  ...
  var Position = function (pos, deltaX, deltaY) {
    ...
    if (
      ...
      !(isNumber(deltaX) && isDirection(deltaX))
    )
      throw new Error(
    ...
    if (this.x < 0 || this.x >= width || this.y < 0 || this.y >= height)
      throw new Error(
        "Position constructed with bad bounds " +
          this +
          " " +
          deltaX +
          " " +
    ...
    if (!moveOK)
      throw new Error(
        "Position did not move correctly " + this + " " + deltaX + " " + deltaY
      );
  };
