...
export class TrafficBase extends THREE.Group {
  constructor(o = {}) {
    ...
    this.inter_geo.applyMatrix4(
      new THREE.Matrix4().makeRotationX(-Math.PI * 0.5)
    );

    this.road_geo = new THREE.PlaneGeometry(this.grid, this.grid);
    this.road_geo.applyMatrix4(
      new THREE.Matrix4().makeRotationX(-Math.PI * 0.5)
    );

    this.mats["inter_mat"] = new MATYPE({
      map: loader.load(this.mapPath + "roadx.png"),
      ...op,
    });
    this.mats["road_mat"] = new MATYPE({
      map: loader.load(this.mapPath + "road.png"),
      ...op,
    });
  ...
  addRoad(road) {
    ...
    if (this.roads[id] == null) {
      ...
      while (i--) {
        if (side == 0) {
          m.makeTranslation(
            p0.x + this.grid * dir + i * this.grid * dir,
            0,
            p0.y
          );
          ...
          m.makeTranslation(
            p0.x,
            0,
            p0.y + this.grid * dir + i * this.grid * dir
          );
        }
  ...
  addCar(car) {
    ...
    if (this.cars[id] == null) {
      ...
      let c = new THREE.Mesh(
        this.car_geo[Traffic.TYPE_OF_CARS[car.type].m],
        this.car_mat[r]
      );
