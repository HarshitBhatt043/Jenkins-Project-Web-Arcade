...
export class Trajectory {
  ...
  getDistanceToIntersection() {
    let distance;
    distance =
      this.current.lane.length - this.car.length * 0.5 - this.current.position;
  ...
  moveForward(distance) {
    ...
    tempRelativePosition =
      this.temp.position /
      ((_ref = this.temp.lane) != null ? _ref.length : void 0);
    ...
    if (
      ...
      this.temp.position + gap >
        ((_ref1 = this.temp.lane) != null ? _ref1.length : void 0)
    )
      this.next.acquire();
  ...
  changeLane(nextLane) {
    ...
    if (!(nextPosition < this.lane.length))
      nextPosition = this.current.position + 2 * this.car.length;
    if (!(nextPosition < this.lane.length))
      nextPosition = this.current.position + 1 * this.car.length;
    if (!(nextPosition < this.lane.length))
      nextPosition = this.current.position;
  ...
  _getAdjacentLaneChangeCurve() {
    ...
    direction1 = this.current.lane.middleLine.vector.normalized.mult(
      distance * 0.3
    );
    control1 = p1.add(direction1);
    direction2 = this.next.lane.middleLine.vector.normalized.mult(
      distance * 0.3
    );
