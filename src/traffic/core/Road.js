...
export class Road {
  ...
  update() {
    ...
    this.targetSide = this.target.rect
      ...
      .subsegment(0, 0.5);
    this.lanesNumber =
      Traffic.min(this.sourceSide.length, this.targetSide.length) | 0;
    this.lanesNumber = Traffic.max(
      2,
      (this.lanesNumber / Traffic.settings.gridSize) | 0
    );
    ...
    if (this.lanes == null || this.lanes.length < this.lanesNumber) {
      ...
      for (
        i = _i = 0, _ref = this.lanesNumber - 1;
    ...
    _results = [];
    for (
      i = _j = 0, _ref1 = this.lanesNumber - 1;
