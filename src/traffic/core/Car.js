...
export class Car {
  constructor(lane, position) {
    ...
    if (Traffic.TYPE_OF_CARS[this.type].maxSpeed)
      this.maxSpeed =
        Traffic.TYPE_OF_CARS[this.type].maxSpeed * Traffic.settings.carSpeed;
  ...
  getAcceleration() {
    ...
    deltaSpeed =
      this.speed -
        ((_ref = nextCarDistance.car) != null ? _ref.speed : void 0) || 0;
    ...
    safeIntersectionDistance =
      this.s1 + timeGap + Traffic.pow(this.speed, 2) / (2 * b);
    intersectionCoeff = Traffic.pow(
  ...
  move(delta) {
    ...
    if (this.trajectory.nextCarDistance.distance < step) {
      /*this.alive = false;*/ console.log("bad IDM");
    }
