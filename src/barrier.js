...
MG.Barrier = function (type) {
  ...
  var mType =
    type === MG.BarrierType.RANDOM
      ? Math.ceil(MG.NUM_RANDOM_BARRIERS * Math.random())
      : type;
  ...
  this.collides = function (x, y) {
    ...
    var x_ =
      x * Math.cos((mTheta * Math.PI) / 180) +
      y * Math.sin((mTheta * Math.PI) / 180);
    var y_ =
      -x * Math.sin((mTheta * Math.PI) / 180) +
      y * Math.cos((mTheta * Math.PI) / 180);
  ...
  this.updateDOM = function (x, y, offset) {
    ...
    offset = Math.max(
      ...
      Math.min(MG.LINE_OF_SIGHT, offset)
    );
    var fog = 100 - (100 * (MG.LINE_OF_SIGHT - offset)) / MG.BARRIER_SPACING;

    mFrontPath.setAttribute(
      "fill",
      "rgb(" + (100 + fog) + "%," + (100 + fog) + "%," + (100 + fog) + "%)"
    );
    mBackPath.setAttribute(
      "fill",
      "rgb(" + (60 + fog) + "%," + (60 + fog) + "%," + (60 + fog) + "%)"
    );
    mFrontPath.setAttribute(
      "stroke",
      "rgb(" + (0 + fog) + "%," + (0 + fog) + "%," + (0 + fog) + "%)"
    );
    mBackPath.setAttribute(
      "stroke",
      "rgb(" + (0 + fog) + "%," + (0 + fog) + "%," + (0 + fog) + "%)"
