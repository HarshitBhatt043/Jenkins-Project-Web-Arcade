...
class Position {
  constructor(pos, deltaX, deltaY) {
    ...
    if (arguments.length === 2 && this.isNumber(pos) && !this.isNumber(deltaX))
      throw new Error(
  ...
  hospitalFound: function (map, x, y, simData) {
    ...
    if (simData.census.needHospital === -1) {
      if (math.getRandom(20) === 0)
        ZoneUtils.putZone(map, x, y, Tile.FREEZ, map.getTile(x, y).isPowered());
    }
  },
};
  ...
  industrialFound: function (map, x, y, simData) {
    ...
    if (math.getChance(7)) {
      var zoneScore =
        ...
        (trafficOK === Micro.NO_ROAD_FOUND ? -1000 : 0);
...
const Road = {
  ...
  roadFound: function (map, x, y, simData) {
    ...
    let currentDensity =
      simData.blockMaps.trafficDensityMap.worldGet(x, y) >> 6;
    // Force currentDensity in range 0-3 (trafficDensityMap values are capped at 240)
    if (currentDensity > 1) currentDensity -= 1;
    if (currentDensity === density) return;

    let newValue =
      ((tileValue - Tile.ROADBASE) & 15) + densityTable[currentDensity];
    ...
    while (x--) {
      ...
      while (y--) {
        ...
        if (developed) {
          landValue =
            34 -
            Math.floor(MapUtils.getCityCentreDistance(map, worldX, worldY) / 2);
...
class Simulation {
  ...
  simulate() {
    ...
    switch (this.phaseCycle) {
      ...
      case 13:
        ...
        break;
      case 14:
        if (this.simCycle % Micro.speedPopulationDensityScan[speedIndex] === 0)
          MapUtils.populationDensityScan(this.map, this.blockMaps);
        break;
      case 15:
  ...
  sendMessages() {
    this.checkGrowth();
    let totalZonePop =
      this.census.resZonePop + this.census.comZonePop + this.census.indZonePop;
    ...
    switch (this.cityTime & 63) {
      ...
      case 57:
        if (
          ...
          this.messageManager.sendMessage(Messages.FIRE_STATION_NEEDS_FUNDING);
        break;
      case 60:
        if (
      ...
      case 63:
        if (this.census.trafficAverage > 60)
          this.messageManager.sendMessage(Messages.TRAFFIC_JAMS, -1, -1, true);
        break;
    }
  }
...
class GameMap {
  ...
  removeTileFlags(x, y, flags) {
    if (arguments.length < 2)
      throw new Error("GameMap removeTileFlags called with too few arguments"); // Argument-shuffling
    if (arguments.length === 2) {
      flags = y;
      y = x.y;
      x = x.x;
...
class CityGame {
  static message(e) {
    ...
    if (p == "MAPCLICK")
      Game.mapClick(e.data.x, e.data.y, e.data.single || false);
