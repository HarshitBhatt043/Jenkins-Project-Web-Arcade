...
(function (global, factory) {
  ...
  class ZoneUtils {
    ...
    static canBulldoze(tile) {
      ...
      return (
        (tile >= Tile.FIRSTRIVEDGE && tile <= Tile.LASTRUBBLE) ||
    ...
    static isDriveable(tile) {
      tile = ZoneUtils.unwrapTile(tile);
      return (
        (tile >= Tile.ROADBASE && tile <= Tile.LASTRAIL) ||
        tile === Tile.RAILHPOWERV ||
        tile === Tile.RAILVPOWERH
      );
    }
    ...
    static normalizeRoad(tile) {
      tile = ZoneUtils.unwrapTile(tile);
      return tile >= Tile.ROADBASE && tile <= Tile.LASTROAD + 1
        ? (tile & 15) + 64
        : tile;
    } ///
    ...
    static checkZoneSize(tile) {
      if (
        (tile >= Tile.RESBASE - 1 && tile <= Tile.PORTBASE - 1) ||
      ...
      if (
        (tile >= Tile.PORTBASE && tile <= Tile.LASTPORT) ||
    ...
    static fireZone(map, x, y, blockMaps) {
      ...
      else if (tileValue < Tile.PORTBASE) zoneSize = 2; // Make remaining tiles of the zone bulldozable
  ...
  class Tiles {
    ...
    setValue(tileValue) {
      ...
      if (tileValue < Tile.BIT_START)
        existingFlags = this._value & Tile.ALLBITS; //this.getFlags();
    ...
    addFlags(bitMask) {
      if (
        ...
        bitMask < Tile.BIT_START ||
        bitMask >= Tile.BIT_END << 1
      )
        throw new Error("Invalid parameter");
      this._value |= bitMask;
    }
  ...
  var disasterMessages = [
    Messages.EARTHQUAKE,
    Messages.EXPLOSION_REPORTED,
    Messages.FIRE_REPORTED,
    Messages.FLOODING_REPORTED,
    Messages.MONSTER_SIGHTED,
    Messages.NUCLEAR_MELTDOWN,
    Messages.TORNADO_SIGHTED,
  ];
  Object.defineProperty(
    Messages,
    "disasterMessages",
    MiscUtils.mcd(disasterMessages)
  );
  var crashes = [
    Messages.HELICOPTER_CRASHED,
    Messages.PLANE_CRASHED,
    Messages.SHIP_CRASHED,
    Messages.TRAIN_CRASHED,
  ];
  ...
  const Text = function () {
    ...
    neutralMessages[Messages.NEED_POLICE_STATION] =
    ...
    neutralMessages[Messages.ROAD_NEEDS_FUNDING] =
      "Roads deteriorating, due to lack of funds";
    neutralMessages[Messages.POLICE_NEEDS_FUNDING] =
    ...
    var badMessages = {};
    badMessages[Messages.BLACKOUTS_REPORTED] =
    ...
    badMessages[Messages.NOT_ENOUGH_POWER] =
      "Blackouts reported. insufficient power capacity";
    badMessages[Messages.NUCLEAR_MELTDOWN] =
    ...
    badMessages[Messages.TAX_TOO_HIGH] =
    ...
    goodMessages[Messages.REACHED_MEGALOPOLIS] =
      "Population has reached 500,000";
    goodMessages[Messages.REACHED_METROPOLIS] =
  ...
  class SpriteUtils {
    ...
    static checkWet(tileValue) {
      if (
        tileValue === Tile.HPOWER ||
        tileValue === Tile.VPOWER ||
        tileValue === Tile.HRAIL ||
        tileValue === Tile.VRAIL ||
        tileValue === Tile.BRWH ||
        tileValue === Tile.BRWV
      )
        return true;
    ...
    static checkSpriteCollision(s1, s2) {
      return (
        s1.frame !== 0 &&
        s2.frame !== 0 &&
        SpriteUtils.getDistance(s1.x, s1.y, s2.x, s2.y) < 30
      );
    }
  }
    ...
    move(spriteCycle, messageManager, disasterManager, blockMaps) {
      ...
      if ((spriteCycle & 3) === 0) {
        ...
        for (let i = dir; i < dir + 4; i++) {
          ...
          let tileValue = SpriteUtils.getTileValue(
            this.map,
            this.x + this.tileDeltaX[dir2],
            this.y + this.tileDeltaY[dir2]
          );

          if (
            (tileValue >= Tile.RAILBASE && tileValue <= Tile.LASTRAIL) ||
            tileValue === Tile.RAILVPOWERH ||
            tileValue === Tile.RAILHPOWERV
          ) {
  ...
  class BoatSprite extends BaseSprite {
      ...
      if (this.count === 0) {
        ...
        for (dir = startDir; dir < startDir + 8; dir++) {
          ...
          if (this.map.testBounds(x, y)) {
            ...
            if (
              tileValue === Tile.CHANNEL ||
              tileValue === Tile.BRWH ||
              tileValue === Tile.BRWV ||
              this.oppositeAndUnderwater(tileValue, this.dir, frame)
            ) {
              this.newDir = frame;
              this.frame = SpriteUtils.turnTo(this.frame, this.newDir);
              this.dir = frame + 4;
    ...
    oppositeAndUnderwater(tileValue, oldDir, newDir) {
      ...
      if (
        tileValue == Tile.POWERBASE ||
        tileValue == Tile.POWERBASE + 1 ||
        tileValue == Tile.RAILBASE ||
        tileValue == Tile.RAILBASE + 1
      )
        return true;
      return false;
    }
  ...
  class MonsterSprite extends BaseSprite {
    ...
    move(spriteCycle, messageManager, disasterManager, blockMaps) {
      ...
      if (currentDir < 4) {
        ...
        let absDist = SpriteUtils.absoluteDistance(
          this.x,
          this.y,
          this.destX,
          this.destY
        ...
        if (!(math.getRandom16() & 3)) {
          ...
          else frame = this.diagonals2[frame]; // We mung currentDir and frame here to
      ...
      for (let i = 0; i < spriteList.length; i++) {
        ...
        if (
          ...
          (s.type === Micro.SPRITE_AIRPLANE ||
            s.type === Micro.SPRITE_HELICOPTER ||
            s.type === Micro.SPRITE_SHIP ||
            s.type === Micro.SPRITE_TRAIN) &&
  ...
  class AirplaneSprite extends BaseSprite {
    ...
    move(spriteCycle, messageManager, disasterManager, blockMaps) {
      ...
      if (absDist < 50) {
        ...
        this.destY =
          math.getRandom(SpriteUtils.worldToPix(this.map.height)) + 8;
      }
  ...
  class SpriteManager {
    ...
    getSpritesInView(startX, startY, lastX, lastY) {
      ...
      return this.spriteList.filter(function (s) {
        return (
          s.x + s.xOffset >= startX &&
          s.y + s.yOffset >= startY &&
    ...
    makeTornado(messageManager) {
      ...
      let x =
        math.getRandom(SpriteUtils.worldToPix(this.map.width) - 800) + 400;
      let y =
        math.getRandom(SpriteUtils.worldToPix(this.map.height) - 200) + 100;
    ...
    generateTrain(census, x, y) {
      if (
        census.totalPop > 20 &&
        this.getSprite(Micro.SPRITE_TRAIN) === null &&
        math.getRandom(25) === 0
      )
        this.makeSprite(
          Micro.SPRITE_TRAIN,
    ...
    getBoatDistance(x, y) {
      ...
      for (let i = 0, l = this.spriteList.length; i < l; i++) {
        ...
        if (sprite.type === Micro.SPRITE_SHIP && sprite.frame !== 0) {
          ...
          let sprDist =
            Math.abs(sprite.x - pixelX) + Math.abs(sprite.y - pixelY);
  ...
  class EvaluationUtils {
    static getTrafficAverage(blockMaps, census) {
      ...
      for (
        var x = 0;
        x < landValueMap.gameMapWidth;
        x += landValueMap.blockSize
      ) {
        for (
          var y = 0;
  ...
  class Evaluation {
    ...
    evalInit() {
      ...
      for (i = 0; i < Micro.NUM_COMPLAINTS; i++)
        this.problemOrder[i] = Micro.NUMPROBLEMS;
    }
    ...
    getPopulation(census) {
      ...
      if (this.cityPopDelta !== 0)
        EventEmitter.emitEvent(Messages.POPULATION_UPDATED, this.cityPop);
      return this.cityPop;
    }
    ...
    doProblems(census, budget, blockMaps) {
      ...
      Micro.problemData[Micro.TRAFFIC] = EvaluationUtils.getTrafficAverage(
        blockMaps,
        census
      );
      Micro.problemData[Micro.UNEMPLOYMENT] =
        EvaluationUtils.getUnemployment(census);
    ...
    getScore(simData) {
      ...
      if (budget.roadEffect < budget.MAX_ROAD_EFFECT)
        score -= budget.MAX_ROAD_EFFECT - budget.roadEffect; // Penalize player by up to 10% for underfunded police and fire services

      if (budget.policeEffect < budget.MAX_POLICE_STATION_EFFECT) {
        score = Math.round(
          score *
            (0.9 +
              budget.policeEffect / (10 * budget.MAX_POLICE_STATION_EFFECT))
        );
      }
      ...
      if (budget.fireEffect < budget.MAX_FIRE_STATION_EFFECT) {
        score = Math.round(
          score *
            (0.9 + budget.fireEffect / (10 * budget.MAX_FIRE_STATION_EFFECT))
        );
      ...
      if (
        ...
        scale =
          0.95 +
          Math.floor(this.cityPopDelta / (this.cityPop - this.cityPopDelta));
      }
      ...
      if (scale > 0.0)
        score = Math.round(score * (census.poweredZoneCount / scale)); // Force in to range 0-1000. New score is average of last score and new computed value
      ...
      if (this.cityScoreDelta !== 0)
        EventEmitter.emitEvent(Messages.SCORE_UPDATED, this.cityScore);
    }
  ...
  class Valves {
    ...
    setValves(gameLevel, census, budget) {
      ...
      census.totalPop = Math.round(
        normalizedResPop + census.comPop + census.indPop
      ...
      if (census.resPop > 0)
        employment =
          (census.comHist10[1] + census.indHist10[1]) / normalizedResPop;
      else employment = 1; // Given the employment rate, calculate expected migration, add in births, and project the new population.
      ...
      var projectedIndPop =
        census.indPop * labourBase * Micro.extMarketParamTable[gameLevel];
  ...
  class Budget {
    ...
    _calculateBestPercentages() {
      ...
      this.roadSpend = Math.round(
        this.roadMaintenanceBudget * this.roadPercent
      );
      this.fireSpend = Math.round(
        this.fireMaintenanceBudget * this.firePercent
      );
    ...
    collectTax(gameLevel, census) {
      ...
      this.policeMaintenanceBudget =
        census.policeStationPop * Micro.policeMaintenanceCost;
      this.fireMaintenanceBudget =
        census.fireStationPop * Micro.fireMaintenanceCost;
  ...
  class Position {
    constructor(pos, deltaX, deltaY) {
      ...
      if (arguments.length === 1 && !pos.isPosition)
        throw new Error("Position constructor called with invalid pos " + pos);
      if (arguments.length === 3 && !pos.isPosition)
        throw new Error("Position constructor called with invalid pos " + pos);
      if (
        arguments.length === 3 &&
        !(this.isNumber(deltaX) && this.isNumber(deltaY))
      )
        throw new Error(
          "Position constructor called with invalid deltas " +
            deltaX +
            " " +
            deltaY
        );
      if (
        arguments.length === 2 &&
        this.isNumber(pos) &&
        !this.isNumber(deltaX)
      )
        throw new Error(
          "Position constructor called with invalid y coordinate " +
            pos +
            " " +
            deltaX
        );
      if (
        arguments.length === 2 &&
        pos.isPosition &&
        !(this.isNumber(deltaX) && this.isDirection(deltaX))
      )
        throw new Error(
          "Position constructor called with invalid direction " +
            pos +
            " " +
            deltaX
        );
      if (arguments.length === 2 && !this.isNumber(pos) && !pos.isPosition)
        throw new Error(
          "Position constructor called with bad existing position " +
            pos +
            " " +
            deltaX
  ...
  class PowerManager {
    ...
    setTilePower(x, y) {
      ...
      if (
        tileValue === Tile.NUCLEAR ||
        tileValue === Tile.POWERPLANT ||
    ...
    doPowerScan(census) {
      ...
      let maxPower =
        census.coalPowerPop * Micro.COAL_POWER_STRENGTH +
        census.nuclearPowerPop * Micro.NUCLEAR_POWER_STRENGTH;
    ...
    nuclearPowerFound(map, x, y, simData) {
      ...
      if (
        ...
        math.getRandom(meltdownTable[simData.gameLevel]) === 0
      ) {
      ...
      if (!simData.is3D)
        for (var i = 0; i < 4; i++)
          map.addTileFlags(
            x,
            y,
            Tile.ANIMBIT | Tile.CONDBIT | Tile.POWERBIT | Tile.BURNBIT
          );
    }
  ...
  class Traffic {
    ...
    tryGo(pos, dirLast) {
      ...
      for (i = 0; i < 4; i++) {
        if (
          dir != dirLast &&
          ZoneUtils.isDriveable(
            this._map.getTileFromMapOrDefault(pos, dir, Tile.DIRT)
          )
        ) {
    ...
    doMeltdown(x, y) {
      ...
      for (i = 0; i < 200; i++) {
        ...
        if (tile.isCombustible() || tile.getValue() === Tile.DIRT)
          this._map.setTile(dX, dY, Tile.RADTILE, 0); //this._map.setTo(dX, dY, new Tiles(Tile.RADTILE));
  ...
  const Residential = {
    registerHandlers: function (mapScanner, repairManager) {
      mapScanner.addAction(
        ZoneUtils.isResidentialZone,
    ...
    getZonePopulation: function (map, x, y, tileValue) {
      ...
      if (tileValue === Tile.FREEZ)
        return Residential.getFreeZonePopulation(map, x, y, tileValue);
    ...
    buildHouse: function (map, x, y, lpValue) {
      ...
      if (best > 0 && map.testBounds(x + xDelta[best], y + yDelta[best]))
        map.setTile(
          x + xDelta[best],
          y + yDelta[best],
          Tile.HOUSE + math.getRandom(2) + lpValue * 3,
          Tile.BLBNCNBIT
        ...
        for (yy = y - 1; yy <= y + 1; yy++) {
          for (xx = x - 1; xx <= x + 1; xx++) {
            ...
            map.setTile(
              x,
              y,
              Tile.LHTHR + lpValue + math.getRandom(2),
              Tile.BLBNCNBIT
    ...
    residentialFound: function (map, x, y, simData) {
      ...
      if (population > math.getRandom(35)) {
        ...
        trafficOK = simData.traffic.makeTraffic(
          x,
          y,
          simData.blockMaps,
          ZoneUtils.isCommercial
      ...
      if (tileValue === Tile.FREEZ || math.getChance(7)) {
        ...
        let locationScore = Residential.evalResidential(
          simData.blockMaps,
          x,
          y,
          trafficOK
    ...
    commercialFound: function (map, x, y, simData) {
      ...
      if (math.getChance(7)) {
        ...
        if (
          zonePower &&
          zoneScore > -350 &&
          zoneScore - 26380 > math.getRandom16Signed()
        ) {
          lpValue = ZoneUtils.getLandPollutionValue(simData.blockMaps, x, y);
          Commercial.growZone(
            map,
  ...
  const Industrial = {
    ...
    industrialFound: function (map, x, y, simData) {
      ...
      if (!simData.is3D)
        Industrial.setAnimation(map, x, y, tileValue, zonePower); // Occasionally check to see if the zone is connected to the transport network (the chance of this happening
      ...
      if (math.getChance(7)) {
        var zoneScore =
          ...
          (trafficOK === Micro.NO_ROAD_FOUND ? -1000 : 0); // Unpowered zones should of course be penalized
  ...
  const Road = {
    ...
    roadFound: function (map, x, y, simData) {
      ...
      let currentDensity =
        simData.blockMaps.trafficDensityMap.worldGet(x, y) >> 6; // Force currentDensity in range 0-3 (trafficDensityMap values are capped at 240)

      if (currentDensity > 1) currentDensity -= 1;
      if (currentDensity === density) return;
      let newValue =
        ((tileValue - Tile.ROADBASE) & 15) + densityTable[currentDensity]; // Preserve all bits except animation
  ...
  const Transport = {
      ...
      if (tile.isPowered()) {
        if (map.getTileValue(x + 1, y - 1) === Tile.RADAR)
          map.setTile(
            ...
            y - 1,
            Tile.RADAR0,
            Tile.CONDBIT | Tile.ANIMBIT | Tile.BURNBIT
          );
    ...
    portFound: function (map, x, y, simData) {
      ...
      if (
        tile.isPowered() &&
        simData.spriteManager.getSprite(Micro.SPRITE_SHIP) === null
      )
  ...
  const EmergencyServices = {
    ...
    policeStationFound: handleService(
      "policeStationPop",
      "policeEffect",
      "policeStationMap"
    ),
    fireStationFound: handleService(
      "fireStationPop",
      "fireEffect",
      "fireStationMap"
    ),
  ...
  const MiscTiles = {
    ...
    fireFound: function (map, x, y, simData) {
      ...
      else if (i > 0) rate = 3; // Decide whether to put out the fire.
  ...
  class MapUtils {
    ...
    static pollutionTerrainLandValueScan(map, census, blockMaps) {
        ...
        while (y--) {
          ...
          if (developed) {
            landValue =
              34 -
              Math.floor(
                MapUtils.getCityCentreDistance(map, worldX, worldY) / 2
              );
      ...
      if (developedTileCount > 0)
        census.landValueAverage = Math.floor(
          totalLandValue / developedTileCount
        );
      else census.landValueAverage = 0; // Smooth the pollution map twice
        ...
        for (y = 0; y < map.height; y += pollutionDensityMap.blockSize) {
          ...
          if (pollution !== 0) {
            ...
            if (
              ...
              (pollution === maxPollution && math.getChance(3))
            ) {
      ...
      if (pollutedTileCount)
        census.pollutionAverage = Math.floor(
          totalPollution / pollutedTileCount
        );
    ...
    static fillCityCentreDistScoreMap(map, blockMaps) {
      ...
      while (x--) {
        ...
        while (y--) {
          ...
          value = Math.floor(
            MapUtils.getCityCentreDistance(map, x * 8, y * 8) / 2
      ...
      while (x--) {
        ...
        while (y--) {
          ...
          if (tile.isZone()) {
            ...
            population =
              MapUtils.getPopulationDensity(map, x, y, tileValue) * 8;
  ...
  class Simulation {
    constructor(gameMap, gameLevel, speed, is3D, savedGame) {
      if (
        gameLevel !== Micro.LEVEL_EASY &&
        gameLevel !== Micro.LEVEL_MED &&
        gameLevel !== Micro.LEVEL_HARD
      )
        throw new Error("Invalid level!"); // if (speed !== Micro.SPEED_PAUSED && speed !== Micro.SPEED_SLOW && speed !== Micro.SPEED_MED && speed !== Micro.SPEED_FAST) throw new Error('Invalid speed!');
    ...
    load(saveData) {
      ...
      for (let i = 0, l = Micro.savePropsVar.length; i < l; i++)
        this[Micro.savePropsVar[i]] = saveData[Micro.savePropsVar[i]]; //this.map.load(saveData);
    ...
    updateInfo() {
      this.infos[0] = [
        TXT.months[this._cityMonthLast],
        this._cityYearLast,
      ].join(" ");
    ...
    init() {
      ...
      MapUtils.pollutionTerrainLandValueScan(
        this.map,
        this.census,
        this.blockMaps
      );
    ...
    simulate() {
      ...
      switch (this.phaseCycle) {
        ...
        case 7:
        case 8:
          this.mapScanner.mapScan(
            (this.phaseCycle - 1) * this.div,
            this.phaseCycle * this.div,
            null
          );
          break;
        ...
        case 11:
          if (this.simCycle % Micro.speedPowerScan[speedIndex] === 0)
            this.powerManager.doPowerScan(this.census);
          break;
        ...
        case 12:
          if (
            this.simCycle %
              Micro.speedPollutionTerrainLandValueScan[speedIndex] ===
            0
          )
            MapUtils.pollutionTerrainLandValueScan(
              this.map,
        ...
        case 13:
          if (this.simCycle % Micro.speedCrimeScan[speedIndex] === 0)
            MapUtils.crimeScan(this.census, this.blockMaps);
          break;

        case 14:
          if (
            this.simCycle % Micro.speedPopulationDensityScan[speedIndex] ===
            0
          )
            MapUtils.populationDensityScan(this.map, this.blockMaps);
          break;
        ...
        case 15:
          if (this.simCycle % Micro.speedFireAnalysis[speedIndex] === 0)
    ...
    sendMessages() {
      this.checkGrowth();
      let totalZonePop =
        this.census.resZonePop +
        this.census.comZonePop +
        this.census.indZonePop;
      let powerPop = this.census.nuclearPowerPop + this.census.coalPowerPop;

      switch (this.cityTime & 63) {
        case 1:
          if (Math.floor(totalZonePop / 4) >= this.census.resZonePop)
            this.messageManager.sendMessage(Messages.NEED_MORE_RESIDENTIAL);
          break;

        case 5:
          if (Math.floor(totalZonePop / 8) >= this.census.comZonePop)
            this.messageManager.sendMessage(Messages.NEED_MORE_COMMERCIAL);
          break;

        case 10:
          if (Math.floor(totalZonePop / 8) >= this.census.indZonePop)
            this.messageManager.sendMessage(Messages.NEED_MORE_INDUSTRIAL);
          break;

        case 14:
          if (totalZonePop > 10 && totalZonePop * 2 > this.census.roadTotal)
            this.messageManager.sendMessage(Messages.NEED_MORE_ROADS);
          break;

        case 18:
          if (totalZonePop > 50 && totalZonePop > this.census.railTotal)
            this.messageManager.sendMessage(Messages.NEED_MORE_RAILS);
          break;

        case 22:
          if (totalZonePop > 10 && powerPop == 0)
            this.messageManager.sendMessage(Messages.NEED_ELECTRICITY);
          break;
        ...
        case 32:
          let zoneCount =
            this.census.unpoweredZoneCount + this.census.poweredZoneCount;

          if (zoneCount > 0) {
            if (this.census.poweredZoneCount / zoneCount < 0.7)
              this.messageManager.sendMessage(Messages.BLACKOUTS_REPORTED);
          }
          break;

        case 35:
          if (this.census.pollutionAverage > 60)
            this.messageManager.sendMessage(Messages.HIGH_POLLUTION);
          break;

        case 42:
          if (this.census.crimeAverage > 100)
            this.messageManager.sendMessage(Messages.HIGH_CRIME);
          break;

        case 45:
          if (this.census.totalPop > 60 && this.census.fireStationPop === 0)
            this.messageManager.sendMessage(Messages.NEED_FIRE_STATION);
          break;

        case 48:
          if (this.census.totalPop > 60 && this.census.policeStationPop === 0)
            this.messageManager.sendMessage(Messages.NEED_POLICE_STATION);
          break;

        case 51:
          if (this.budget.cityTax > 12)
            this.messageManager.sendMessage(Messages.TAX_TOO_HIGH);
          break;
        ...
        case 54:
          if (
            ...
            this.census.roadTotal > 30
          )
            this.messageManager.sendMessage(Messages.ROAD_NEEDS_FUNDING);
          break;
        ...
        case 60:
          if (
            ...
            this.messageManager.sendMessage(Messages.POLICE_NEEDS_FUNDING);
          break;

        case 63:
          if (this.census.trafficAverage > 60)
            this.messageManager.sendMessage(
              Messages.TRAFFIC_JAMS,
              -1,
              -1,
              true
            );
  ...
  class GameMap {
    ...
    getTileValue(x, y) {
      ...
      if (!this.testBounds(x, y)) throw e;
      let tileIndex = this.getId(x, y);
      if (!(tileIndex in this.data))
        this.data[tileIndex] = new Tiles(this.defaultValue);
    ...
    putZone(centreX, centreY, centreTile, size) {
      if (
        ...
        !this.testBounds(centreX - 1 + size, centreY - 1 + size)
      )
        throw new Error("GameMap putZone called with invalid bounds");
      ...
      for (y = startY; y < startY + size; y++) {
        for (x = startX; x < startX + size; x++) {
          if (x === centreX && y === centreY)
            this.setTo(x, y, new Tiles(tile, Tile.BNCNBIT | Tile.ZONEBIT));
  ...
  class MapGenerator {
    ...
    construct(w, h, debug = false) {
      ...
      else this.clearMap(); // Lay a river.
    ...
    cleanBorder() {
      ...
      for (x = 0; x < map.width; x++) {
        ...
        s = 1;
        if (
          map.getTileValue(x, s) +
            map.getTileValue(l, s) +
            map.getTileValue(r, s) ===
          6
        )
          map.setTile(x, 0, Tile.RIVER, 0);
        s = map.height - 2;
        if (
          map.getTileValue(x, s) +
            map.getTileValue(l, s) +
          ...
          map.setTile(x, map.height - 1, Tile.RIVER, 0);
      }
      ...
      for (y = 0; y < map.height; y++) {
        ...
        s = 1;
        if (
          map.getTileValue(s, y) +
            map.getTileValue(s, l) +
            map.getTileValue(s, r) ===
          6
        )
          map.setTile(0, y, Tile.RIVER, 0);
        s = map.width - 2;
        if (
          map.getTileValue(s, y) +
            map.getTileValue(s, l) +
          ...
          map.setTile(map.width - 1, y, Tile.RIVER, 0);
      }
    }
    ...
    makeLakes() {
      let x, y;
      let numLakes =
        Micro.TERRAIN_LAKE_LEVEL < 0
          ? math.getRandom(10)
          : Micro.TERRAIN_LAKE_LEVEL * 0.5;
    ...
    treeSplash(x, y) {
      let numTrees =
        Micro.TERRAIN_TREE_LEVEL < 0
          ? math.getRandom(150) + 50
          : math.getRandom(100 + Micro.TERRAIN_TREE_LEVEL * 2) + 50;
    ...
    doTrees() {
      let i =
        Micro.TERRAIN_TREE_LEVEL < 0
          ? math.getRandom(100) + 50
          : Micro.TERRAIN_TREE_LEVEL + 3;

      while (i--) {
        this.treeSplash(
          math.getRandom(this.map.width - 1),
          math.getRandom(this.map.height - 1)
        );
      }
      /*for (var x = 0; x < i; x++) {
        ...
        if (map.getTileValue(x, y) === Tile.REDGE) {
          ...
          for (z = 0; z < 4; z++) {
            ...
            if (map.testBounds(xt, yt)) {
              ...
              if (
                tt !== Tile.DIRT &&
                (tt < Tile.WOODS_LOW || tt > Tile.WOODS_HIGH)
              )
                bitIndex++;
            }
          }
    ...
    smoothTrees() {
      ...
      map.pp.forEach((v) => {
        x = v[0];
        y = v[1];
        if (this.isTree(map.getTileValue(x, y)))
    ...
    smoothTreesAt(x, y, preserve) {
      ...
      for (i = 0; i < 4; i++) {
        ...
        if (
          ...
          this.isTree(map.getTileValue(xTemp, yTemp))
        )
          bitIndex++;
      }
  ...
  class WorldEffects {
    ...
    setTile(x, y, value, flags) {
      if (flags !== undefined && value.isTile)
    ...
    fixSingle(x, y) {
      ...
      if (tile >= Tile.ROADS && tile <= Tile.INTERSECTION) {
        if (y > 0) {
          ...
          if (
            (tile === Tile.HRAILROAD ||
              (tile >= Tile.ROADBASE && tile <= Tile.VROADPOWER)) &&
            tile !== Tile.HROADPOWER &&
            tile !== Tile.VRAILROAD &&
            tile !== Tile.ROADBASE
          )
            adjTile |= 1;
        ...
        this._worldEffects.setTile(
          x,
          y,
          RoadTable[adjTile] | Tile.BULLBIT | Tile.BURNBIT
        );
      ...
      if (tile >= Tile.LHRAIL && tile <= Tile.LVRAIL10) {
        ...
        this._worldEffects.setTile(
          x,
          y,
          RailTable[adjTile] | Tile.BULLBIT | Tile.BURNBIT
        );
  ...
  class BulldozerTool extends BaseToolConnector {
          ...
          if (this._map.testBounds(xx, yy)) {
            ...
            if (tile !== Tile.RADTILE && tile !== Tile.DIRT) {
              this._worldEffects.setTile(
                xx,
                yy,
                Tile.TINYEXP + math.getRandom(2),
                Tile.ANIMBIT | Tile.BULLBIT
              );
            }
          }
        }
    ...
    doTool(x, y, blockMaps, messageManager) {
      ...
      if (
        tileValue === Tile.RIVER ||
        tileValue === Tile.REDGE ||
        tileValue === Tile.CHANNEL
      ) {
      ...
      switch (tile) {
        case Tile.DIRT:
          this._worldEffects.setTile(
            x,
            y,
            Tile.LHRAIL | Tile.BULLBIT | Tile.BURNBIT
          );
        ...
        case Tile.LHPOWER:
          this._worldEffects.setTile(
            ...
            y,
            Tile.RAILVPOWERH,
            Tile.CONDBIT | Tile.BURNBIT | Tile.BULLBIT
          );
      ...
      switch (tile) {
        ...
        case Tile.LHPOWER:
          this._worldEffects.setTile(
            x,
            y,
            Tile.VROADPOWER | Tile.CONDBIT | Tile.BURNBIT | Tile.BULLBIT
          );
        ...
        case Tile.LVPOWER:
          this._worldEffects.setTile(
            x,
            y,
            Tile.HROADPOWER | Tile.CONDBIT | Tile.BURNBIT | Tile.BULLBIT
          );
        ...
        case Tile.LHRAIL:
          this._worldEffects.setTile(
            x,
            y,
            Tile.HRAILROAD | Tile.BURNBIT | Tile.BULLBIT
          );
        ...
        case Tile.LVRAIL:
          this._worldEffects.setTile(
            x,
            y,
            Tile.VRAILROAD | Tile.BURNBIT | Tile.BULLBIT
          );
  ...
  class QueryTool extends BaseTool {
    ...
    classifyLandValue(x, y, blockMaps) {
      ...
      else if (landValue >= 30) i = 1; //var text = TXT.landValueStrings[i];
  ...
  class CityGame {
    static message(e) {
      ...
      if (p == "MAPCLICK")
        Game.mapClick(e.data.x, e.data.y, e.data.single || false); //if( p == "DESTROY" ) Game.destroy(e.data.x, e.data.y);
      ...
      if (p == "MAKELOADGAME")
        Game.makeLoadGame(e.data.savegame, e.data.isStart);
    }
  ...
  class MainGame {
    ...
    animatedTiles() {
      var animTiles = this.animationManager.getTiles(
        0,
        0,
        this.mapSize[0] + 1,
        this.mapSize[1] + 1,
    ...
    calculateSprites() {
      this.sprites = this.simulation.spriteManager.getSpritesInView(
        0,
        0,
        this.mapSize[0] + 1,
        this.mapSize[1] + 1
      ...
      for (var i = 0, l = messages.length; i < l; i++) {
        ...
        switch (m.message) {
          ...
          default:
            ...
            if (
              !messageOutput &&
              TXT.neutralMessages[m.message] !== undefined
            ) {
              messageOutput = true;
