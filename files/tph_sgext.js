...
function sg_spil_ad() {
  try {
    if (sg_spilapi_obj) {
      sg_spilapi_obj.GameBreak.request(
...
function sg_spil_splash() {
  try {
    if (sg_spilapi_obj) {
      sg_spilapi_obj.Branding.displaySplashScreen(
...
function sg_fgl_spon_create() {
  try {
    if (window.sg_fgl_spon_obj == undefined) {
      ...
      sg_fgl_spon_obj.setAttribute(
        "style",
        "display: none; position: absolute; left: 0px; top: 0px; width: 0px; height: 0px; z-index: 5; border: 0px"
      );
...
function sg_banner_create(siz) {
  try {
    ...
    bancont.srcdoc =
      '<script type="text/javascript" src="http://ad.leadboltmobile.net/show_app_ad.js?section_id=191234692"></script>';
  ...
  try {
    if (window.adscreen != undefined) {
      ...
      if (window.adcont == undefined) {
        ...
        adcont.srcdoc =
          '<script type="text/javascript" src="http://ad.leadboltmobile.net/show_app_ad.js?section_id=568879653"></script>';
...
function sg_no_bar() {
  if (!window.location.hash) {
    ...
    if (divh < window.outerHeight + 200) {
      document.getElementById("gm4html5_div_id").style.height =
        window.outerHeight + 200 + "px";
    }
    setTimeout(function () {
      window.scrollTo(0, 1);
    }, 50);
...
function sg_save(name, numb) {
  try {
    ...
    if (sg_save_ok) {
      ...
      document.cookie =
        key + "=" + numb + "; path=/; expires=" + date.toUTCString();
    }
  } catch (e) {}
}
