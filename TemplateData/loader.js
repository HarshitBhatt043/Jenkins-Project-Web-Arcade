...
function InitExternEval(appid) {
  ...
  if (idnet_appid) {
    ...
    xhttp.onreadystatechange = function () {
      if (xhttp.readyState == 4 && xhttp.status == 200) {
        try {
          ...
          if (!data.success) {
            console.log(
              "failed to get client options for appid - " + idnet_appid
            );
            return;
          }
    ...
    xhttp.open(
      "GET",
      "https://account.y8.com/api/v1/json/client_options/" + idnet_appid,
      true
    );
    xhttp.send();
  } else {
...
window.Y8ExternEval = function (_allowGamePause) {
  ...
  if (clientOptionsLoadedEnum != "loaded") return;
...
function createAdDisplayContainer() {
  ...
  adDisplayContainer = new google.ima.AdDisplayContainer(
    document.getElementById("adv_adContainer"),
    videoContent
  );
}
